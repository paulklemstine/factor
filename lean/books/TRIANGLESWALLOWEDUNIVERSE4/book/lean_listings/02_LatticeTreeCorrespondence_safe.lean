...
/-- M3^(-1) subtracts 2n from m, corresponding to quotient q = 2
...
/-- M1^(-1) swaps and transforms: (m, n) ? (n, 2n - m). -/
...
of quotients as the Euclidean algorithm. M3^(-1) reduces m by 2n (preserving n),
and M1^(-1) implements the swap step. Together they execute the continued
fraction expansion of m/n.

CONSEQUENCE: Pythagorean tree factoring is ?(?N) for balanced semiprimes. -/
