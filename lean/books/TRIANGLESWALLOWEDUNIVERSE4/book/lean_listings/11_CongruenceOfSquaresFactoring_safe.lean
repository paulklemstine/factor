...
theorem relations_exceed_base_gives_dependency
    ...
    exists  S : Finset (Fin (k + 1)), S.Nonempty /\
      forall  j : Fin k, ? i in S, relations i j = 0 := by
  by_contra h;
  ...
  have h_pigeonhole : exists  (s : Fin (k + 1) -> ZMod 2), s != 0 /\ ? i, s i ? relations i = 0 := by
    have h_pigeonhole : exists  (s : Fin (k + 1) -> ZMod 2), s != 0 /\ ? i, s i ? relations i = 0 := by
...
-- [... 10 more lines omitted for brevity ...]
-- See the full source in lean/11_CongruenceOfSquaresFactoring.lean