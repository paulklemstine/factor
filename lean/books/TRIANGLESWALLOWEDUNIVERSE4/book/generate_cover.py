...
def draw_tree_branch(draw, x, y, length, angle, depth, max_depth):
    ...
    draw.line([(int(x), int(y)), (int(x2), int(y2))], fill=color, width=max(1, 4 - depth))
...
# Fonts
try:
    font_large = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf", 120)
    font_title = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf", 90)
    font_subtitle = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif-Italic.ttf", 48)
    font_author = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf", 56)
    font_spine = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf", 36)
    font_back = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSerif.ttf", 36)
...
for offset in range(8, 0, -1):
    ...
    draw.text((front_cx - tw//2 - offset, front_cy - 60 - offset), eq_text, fill=glow_color, font=font_large)
    draw.text((front_cx - tw//2 + offset, front_cy - 60 + offset), eq_text, fill=glow_color, font=font_large)
draw.text((front_cx - tw//2, front_cy - 60), eq_text, fill=(255, 255, 255), font=font_large)
...
for t, yoff in [(title1, -500), (title2, -390)]:
    ...
    for off in range(5, 0, -1):
        draw.text((front_cx - tw//2 + off, front_cy + yoff + off), t, fill=(100, 80, 0), font=font_title)
    draw.text((front_cx - tw//2, front_cy + yoff), t, fill=(255, 215, 100), font=font_title)
...
for t, yoff in [(sub, 200), (sub2, 270)]:
    ...
    tw = bb[2] - bb[0]
    draw.text((front_cx - tw//2, front_cy + yoff), t, fill=(200, 220, 255), font=font_subtitle)

# Author
author = "Paul Klemstine"
bb = draw.textbbox((0,0), author, font=font_author)
tw = bb[2] - bb[0]
draw.text((front_cx - tw//2, TOTAL_H - 400), author, fill=(255, 215, 100), font=font_author)
...
tw = bb[2] - bb[0]
spine_draw.text((TOTAL_H//2 - tw//2, W_SPINE//2 - 20), spine_text, fill=(255, 215, 100), font=font_spine)
author_spine = "Paul Klemstine"
bb2 = spine_draw.textbbox((0,0), author_spine, font=font_small)
tw2 = bb2[2] - bb2[0]
spine_draw.text((TOTAL_H - tw2 - 100, W_SPINE//2 - 16), author_spine, fill=(200, 200, 200), font=font_small)
...
for line in blurb_lines:
    if line:
        ...
        tw = bb[2] - bb[0]
        draw.text((back_cx - tw//2, y_pos), line, fill=(220, 220, 240), font=font_back)
    y_pos += 50
...
try:
    ...
    bx = back_cx - bc_w // 2
    by = TOTAL_H - 550
    draw.rectangle([bx - 20, by - 20, bx + bc_w + 20, by + bc_h + 20], fill=(255, 255, 255))
    img.paste(barcode, (bx, by))
    print("Barcode placed successfully")
except Exception as e:
    print(f"Barcode paste failed: {e}")

img.save('/workspace/request-project/book/cover_casewrap.png', 'PNG', dpi=(300, 300))
