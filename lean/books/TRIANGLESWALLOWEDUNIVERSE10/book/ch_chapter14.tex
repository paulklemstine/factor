...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig01_trivial_triple.png}
\caption{A single right triangle with legs labeled $N$ and $(N^2-1)/2$ and hypotenuse $(N^2+1)/2$.}
\end{figure}
...
Pierre de Fermat\index{Fermat, Pierre de}, the seventeenth-century magistrate and prince of amateurs, knew this trick well. Around 1643 he proposed his "method of factoring by difference of squares\index{difference of squares}": if you can write $N = x^2 - y^2 = (x-y)(x+y)$, you have cracked $N$ into factors. Our crowbar is the same idea in Pythagorean clothing. Fermat would have approved.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig02_crowbar.png}
\caption{A visual "crowbar" diagram.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig03_berggren_tree.png}
\caption{A ternary tree diagram, three levels deep, rooted at $(3,4,5)$ at the top.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig04_reverse_climb.png}
\caption{A small inset panel beside the tree.}
\end{figure}
...
Suppose I hand you the triple $(39, 80, 89)$ and ask: which of the three branches produced it? You must figure out its \textit{parent} in the Berggren\index{Berggren, B.} tree\index{Berggren tree}. There is a beautiful fact: you don't need to guess. The arithmetic tells you.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig05_descent_ladder.png}
\caption{A worked example shown as a vertical "ladder." Start at the bottom with the triple $(39, 80, 89)$.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig06_hypotenuse_staircase.png}
\caption{A graph with the horizontal axis labeled "descent step" (showing values $0, 1, 2, 3, \ldots$) and the vertical axis labeled "hypotenuse $c$." A descending staircase of bold dots shows the hypotenuse shrinking at each step, always remaining above the horizontal axis.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig07_three_doors.png}
\caption{A triple $(a, b, c)$ displayed in a central circle.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig08_flowchart.png}
\caption{A flowchart of the factoring algorithm.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig09_worked_77.png}
\caption{A detailed worked example for $N = 77 = 7 \times 11$.}
\end{figure}
...
We already proved this for $B_1^{-1}$ in Section 4, when we showed that ${a'}^2 + {b'}^2 - {c'}^2 = a^2 + b^2 - c^2$. The same algebra goes through for all six maps. The Berggren transformations are \textit{isometries} of the Lorentz\index{Lorentz, Hendrik} form --- symmetries of the light cone that happen to have integer entries.

In the language of group theory, these matrices belong to $O(2,1;\mathbb{Z})$, the \textbf{integer Lorentz group} --- the same mathematical structure that, over the real numbers, governs the coordinate transformations between observers moving at different speeds in Einstein's theory. Over the integers, it governs the coordinate transformations between Pythagorean triple\index{Pythagorean triple}s.
...
Henri Poincaré discovered, in the 1880s, that the interior of such a cone is a model of \textit{hyperbolic geometry\index{hyperbolic geometry}} --- the strange non-Euclid\index{Euclid}ean geometry where parallel lines diverge and triangles have angle sums less than $180°$. The Berggren tree, from this vantage point, is a discrete tiling of the hyperbolic plane by ideal triangles. But that is a story for another chapter.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig10_light_cone.png}
\caption{A three-dimensional wireframe cone, vertex at the origin, with the vertical axis labeled $c$ and the horizontal plane spanned by axes $a$ and $b$.}
\end{figure}
...
How does the number of descent steps compare to trial division? For these small examples, the two methods are roughly comparable --- both take on the order of $\sqrt{N}$ work. The Berggren descent is not a magic bullet for factoring (no one expects it to break RSA\index{RSA cryptosystem}). Its charm lies elsewhere: in the \textit{elegance} of the connection between right triangles and prime factors, in the way the same algebraic identity that fascinated Babylonian scribes four thousand years ago turns out to encode the divisor structure of the integers.

For those interested in complexity, the number of descent steps grows roughly as $\Theta(\sqrt{N})$ for balanced semiprimes\index{semiprime} --- a topic treated in detail in Chapter 8.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig11_five_ladders.png}
\caption{Five side-by-side "descent ladders," one for each of the five examples.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig12_bar_chart.png}
\caption{A bar chart comparing "number of descent steps" (vertical axis) versus $N$ (horizontal axis, showing $15, 21, 77, 143, 323$).}
\end{figure}
...
And beneath it all, like the bass note in an organ chord, lies the \textbf{Lorentz form} $Q(a,b,c) = a^2 + b^2 - c^2$, invariant under every Berggren transformation --- connecting the four-thousand-year-old puzzle of right triangles to the geometry of Einstein's spacetime\index{spacetime}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig13_grand_composite.png}
\caption{A grand, full-page composite image.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter14_fig14_book_map.png}
\caption{A whimsical "map of the book" rendered in the style of a medieval cartographer's chart, with Chapter 14 drawn as a central island.}
\end{figure}
