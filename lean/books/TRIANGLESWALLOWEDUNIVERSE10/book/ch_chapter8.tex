...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig01_locksmith_tree.png}
\caption{A tall, fantastical tree drawn in cross-section, with a tiny locksmith character at the top holding a magnifying glass labeled "$N$." The roots of the tree are labeled with small prime numbers.}
\end{figure}
...
Twelve steps! Every quotient is $1$ except the very last --- the algorithm spirals inward with agonizing slowness, like a nautilus shell winding toward its center. And the GCD, after all that labor, is $1$: the two numbers are coprime\index{coprimality}.
...
The Euclidean algorithm\index{Euclidean algorithm} always terminates in $O(\log(\min(a, b)))$ steps. This is fast --- breathtakingly fast, considering that the algorithm was already ancient when Euclid wrote it down around 300 BCE. It predates the Hindu-Arabic numeral system by over a millennium, yet its efficiency would not be surpassed for two thousand years.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig02_euclid_staircase.png}
\caption{A descending staircase drawn on a grid, where each step represents one division in the Euclidean algorithm applied to $(377, 233)$.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig03_step_count_plot.png}
\caption{A plot of "number of Euclidean algorithm steps" vs.}
\end{figure}
...
Numbers like $10{,}403$ are called \textit{balanced semiprimes\index{semiprime}}: products $N = p \cdot q$ of two primes that are roughly equal in size. They are the hardest nuts for any factoring method to crack, and the reason is geometric.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig04_danger_zone.png}
\caption{A number line from $1$ to $N$, with $p$ and $q$ marked symmetrically around $\sqrt{N}$.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig05_nearly_square.png}
\caption{A square of area $N = p \times q$, drawn as a nearly-square rectangle.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig06_gcd_node.png}
\caption{A single node of the Pythagorean tree, drawn as a circle containing the triple $(a, b, c)$.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig07_descent_clocks.png}
\caption{A tall tree with $\sqrt{N}$ levels, viewed from the side.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig08_multiplication_rect.png}
\caption{A multiplication table–style grid.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig09_complexity_vise.png}
\caption{A vise or clamp squeezing a number from above and below.}
\end{figure}
...
This convergence is not a coincidence. It reflects a deep truth about the problem: *in two dimensions, $\sqrt{N}$ is the natural scale of the factoring problem.* Whether you walk the number line from left to right (trial division\index{trial division}), search outward from the center (Fermat\index{Fermat, Pierre de}), or descend a tree rooted in Pythagorean geometry, you are always navigating a two-dimensional landscape, and the $\sqrt{N}$ barrier is the horizon of that landscape. To go faster, you must escape into higher dimensions.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig10_three_way_race.png}
\caption{Three runners on a racetrack shaped like the number line from $2$ to $\sqrt{N}$.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig11_comparison_table.png}
\caption{A table comparing three factoring methods across three scenarios: (1) $p$ small, $q$ large (Trial Division wins); (2) $p \approx q$ (Fermat wins or ties); (3) General balanced case (all tie at $\sqrt{N}$).}
\end{figure}
...
The dimensional escape is real, and it shatters the $\sqrt{N}$ ceiling. But it comes at the price of enormous mathematical sophistication --- the number field sieve\index{number field sieve} requires algebraic number fields, smooth number recognition, and lattice reduction in dimensions that grow with the input size. The Pythagorean tree, by contrast, is elementary and beautiful. It is a children's garden compared to the industrial landscape of modern factoring. Yet understanding \textit{why} the tree is slow --- understanding the $\sqrt{N}$ barrier and the geometry that creates it --- is the first step toward understanding \textit{how} the barrier is broken.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig12_dimensional_escape.png}
\caption{A two-panel image.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig13_factoring_curves.png}
\caption{A graph of $\log(\text{factoring time})$ vs.}
\end{figure}
...
And there is a deeper question lurking beneath the surface: can higher-dimensional lattice methods, combined with quantum search, break below $N^{1/4}$? The answer, as of the time of this writing, is unknown. Shor\index{Shor, Peter}'s algorithm\index{Shor's algorithm}, of course, factors integers in polynomial time on a quantum computer --- but Shor does not use the Pythagorean tree at all. The question is whether the \textit{geometric} approach, the descent through Pythagorean space, can be enhanced by quantum mechanics to achieve polynomial time. This remains one of the beautiful open problems at the intersection of quantum computation and number theory.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig14_quantum_descent.png}
\caption{A vertical number line from depth $0$ to depth $\sqrt{N}$.}
\end{figure}
...
Is factoring hard because spacetime\index{spacetime} is low-dimensional? Would factoring be easy in a universe with more spatial dimensions? These questions sound like science fiction, but they are precisely the kind of questions that arise when you take the geometry of the Pythagorean tree seriously. The tree has told us something profound: \textit{the cost of descent depends on the curvature of the space you descend through.} And in our two-dimensional slice of the mathematical universe, the price of descent is exactly $\sqrt{N}$ --- not a penny more, not a penny less.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter8_fig15_full_recap.png}
\caption{A full-page image recapitulating the chapter.}
\end{figure}
