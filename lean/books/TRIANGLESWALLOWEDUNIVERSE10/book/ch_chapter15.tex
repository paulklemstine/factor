\chapter{The Algebra Where Two Plus Three Equals Two}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig01_tropical_calculator.png}
\caption{A whimsical blueprint-style diagram of a calculator keypad.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig02_distributive_shift.png}
\caption{A number line running from $0$ to $15$.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig03_totem_pole.png}
\caption{A vertical "totem pole" of integers from $-3$ up to $+5$, with a cloud at the very top labeled "$\infty = $ tropical zero".}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig04_newton_polygon.png}
\caption{A coordinate plane with horizontal axis labeled "exponent $i$" and vertical axis labeled "valuation $a\_i$".}
\end{figure}


This "winner-take-all" property connects directly to \textbf{Newton polygon\index{Newton polygon}s}, a tool that Isaac Newton himself used in his 1676 correspondence with Leibniz. Given a polynomial $\sum a_i x^i$, plot the points $(i, a_i)$ and take the lower convex hull. The slopes of this hull correspond to the roots of the tropicalized polynomial. Newton used the device to approximate roots of equations over three centuries before anyone dreamed of calling it "tropical." One of those beautiful retroactive revelations: a 300-year-old tool turned out to be secretly about an algebra that hadn't been invented yet.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig05_convex_hulls.png}
\caption{Two side-by-side panels.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig06_triangle_inequality.png}
\caption{Three cities ($x$, $y$, $z$) arranged on a road map, with roads of integer length connecting them.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig07_tropical_matrix.png}
...
Richard Bellman\index{Bellman, Richard} was once asked why he named his method "dynamic programming." He confessed that the word "programming" was chosen to camouflage the mathematics from a hostile Secretary of Defense who disliked research. The word "dynamic" was added because no congressman could object to something \textit{dynamic}. Beneath this political theater lay one of the deepest ideas in optimization: the \textbf{principle of optimality}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig08_bellman_ford.png}
\caption{A five-vertex directed graph with source vertex $0$ at left.}
\end{figure}
...
The reader who has followed us from the Berggren\index{Berggren, B.} tree of Pythagorean triple\index{Pythagorean triple}s (Chapter 1) through the hyperbolic plane (Chapter 3) and up to Cayley–Dickson algebras (Chapter 9) may be wondering: what does tropical geometry have to do with factoring integers?

The answer is: everything. The entire book has been about finding shortest paths in disguised graphs.

The \textbf{Berggren tree\index{Berggren tree} descent} of Chapters 1–3 is a shortest-path problem in the Cayley graph of the Lorentz\index{Lorentz, Hendrik} group. Each step down the tree is a "tropical multiplication" --- an additive step in a metric derived from the quadratic form $Q(a,b,c) = a^2 + b^2 - c^2$. The \textbf{lattice reduction} of Chapter 2 --- the Euclidean algorithm computing a shortest vector in a 2D lattice, and its higher-dimensional cousin LLL --- finds its natural framework in tropical convexity. The \textbf{GCD cascade} of Chapter 13 is repeated application of the Bellman equation: at each step, we "relax" the current factor estimate using a new Pythagorean relation. And the tropical distributive law guarantees that multi-channel factor extraction (Chapter 6) works: each independent channel provides a new "edge" in the tropical factor graph, and the minimum over all channels gives the best factor.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig09_book_map.png}
\caption{A large "map of the book" diagram, drawn as a network graph.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter15_fig10_tropical_polynomial.png}
\caption{A piecewise-linear graph of the tropical polynomial $p(x) = \min(3,\; 1+x,\; 4+2x)$, drawn on a standard $xy$-plane.}
\end{figure}
