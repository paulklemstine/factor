...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig01_broken_square.png}
\caption{A right triangle with legs labeled $21$ and $20$ and hypotenuse labeled $29$.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig02_factor_table.png}
\caption{A table with six rows and six columns.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig03_berggren_tree.png}
\caption{An elegant ternary tree, drawn to resemble an actual tree with organic branching.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig04_matrix_multiply.png}
\caption{A "zoom-in" panel showing the explicit matrix-vector multiplication $B\_2 \times (3, 4, 5)^\top = (21, 20, 29)^\top$.}
\end{figure}


This is striking, but the \textit{reason} for the preservation is far more striking still. It has to do with the geometry of spacetime\index{spacetime}.
...
The Berggren tree is not just a clever way to list Pythagorean triples\index{Pythagorean triple}. It is a structure carved from the symmetry group of special relativity.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig05_light_cone.png}
\caption{A three-dimensional coordinate system with axes labeled $a$, $b$, $c$.}
\end{figure}


Let us pause to savor the historical irony. Hermann Minkowski\index{Minkowski, Hermann} introduced the geometry of spacetime in 1908, in a famous lecture at the Cologne Assembly of German Natural Scientists. "Henceforth," he declared, "space by itself, and time by itself, are doomed to fade away into mere shadows, and only a kind of union of the two will preserve an independent reality." The union he described --- Minkowski space, with its indefinite metric $\mathbf{Q}$ --- became the mathematical foundation of Einstein's special relativity\index{special relativity}. Twenty-six years later, Berggren, a schoolteacher an ocean away, discovered three matrices that preserve the same metric. He never mentioned physics; his paper was about Pythagorean triples. The connection between the two discoveries would not be fully appreciated for decades.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig06_photon_blackboard.png}
\caption{A whimsical cartoon of a photon (depicted as a small glowing orb with googly eyes and a mortarboard cap) standing at a blackboard.}
\end{figure}
...
The reason is exactly the Lorentz\index{Lorentz, Hendrik}-preservation equation from the previous section. For any vector $\mathbf{v}$ and any Berggren matrix $B_i$:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig07_q_meter.png}
\caption{A flow diagram.}
\end{figure}
...
Every primitive Pythagorean triple has a unique "postal address" in the Berggren tree\index{Berggren tree} --- a finite string of directions from the alphabet $\{L, M, R\}$, telling you which branches to take on the way down from the root.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig08_subway_map.png}
\caption{A map-style diagram of the Berggren tree's first three levels, drawn as a road network or subway map.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig09_wormhole.png}
\caption{A visual "zoom" effect.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig10_elevator.png}
\caption{The Berggren tree from §2, but now with upward-pointing arrows drawn in a contrasting color (gold against the tree's green).}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig11_lorentz_adjoint.png}
\caption{A side-by-side display of $B\_2$ and $B\_2^{-1}$.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig12_hypotenuse_bars.png}
\caption{A bar chart with three groups of bars, one for each branch (Left, Middle, Right), showing the hypotenuses at levels 0 through 4.}
\end{figure}
...
Pafnuty Lvovich Chebyshev\index{Chebyshev, Pafnuty} (1821–1894) would have appreciated this connection. The great Russian mathematician worked at the intersection of number theory, probability, and approximation theory. His polynomials were born from a question about best approximation --- how closely can a polynomial of degree $n$ approximate a given function? --- but they have infiltrated an astonishing range of mathematics, from mechanical linkage design to signal processing to, as we now see, the internal recurrences of the Berggren tree.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig13_chebyshev_line.png}
\caption{A number line running horizontally across the page, with the middle-branch hypotenuses $5, 29, 169, 985, 5741$ marked at their positions (not to scale --- logarithmic spacing).}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig14_three_subtrees.png}
\caption{Three subtrees of the Berggren tree drawn side by side, one for each branch (Left in red, Middle in blue, Right in green).}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig15_factoring_flowchart.png}
\caption{A flowchart-style diagram, elegant and vertical.}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig16_worked_example.png}
\caption{The specific worked example for $(21, 20, 29)$, laid out as a visual "proof without words." At the top, the right triangle with sides $21$, $20$, $29$.}
\end{figure}
...
In the next chapter, we will shift perspective entirely. If the Berggren tree generates all primitive Pythagorean triples by matrix multiplication, can we understand the same triples through a completely different lens --- the lens of \textit{Gauss\index{Gauss, Carl Friedrich}ian integers\index{Gaussian integers}}, where the Pythagorean equation dissolves into the norm of a complex number? The answer will bring together three seemingly independent roads --- parametric, geometric, and algebraic --- and reveal that they converge at a single crossroads.

But that is a story for another day.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter3_fig17_concept_map.png}
\caption{A full-page "map" of the chapter's conceptual landscape, drawn in the style of a medieval mappa mundi or a fantasy-novel treasure map.}
\end{figure}
