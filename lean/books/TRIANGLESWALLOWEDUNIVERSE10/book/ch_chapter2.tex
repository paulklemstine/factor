...
\begin{quote}
\textit{Can these two machines --- and nothing else --- manufacture every primitive Pythagorean triple\index{Pythagorean triple} that will ever exist?}
\end{quote}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig01_factory_tree.png}
\caption{A sweeping binary tree diagram rooted at the node labeled $(2,1)$, with two branches at each node.}
\end{figure}
...
The last nonzero remainder is $1$, so $\gcd(17, 5) = 1$. The numbers are coprime\index{coprimality}.

This is the \textit{Euclid\index{Euclid}ean algorithm\index{Euclidean algorithm}}, described in Book VII of Euclid's \textit{Elements} around 300 BCE --- though it was almost certainly known to earlier Greek mathematicians, and some scholars suspect the Babylonians had a version of it. Donald Knuth, in \textit{The Art of Computer Programming}, called it "the granddaddy of all algorithms," and with good reason: it is arguably the oldest nontrivial algorithm still in daily use, running inside every computer that performs modular arithmetic, every cryptographic protocol that computes key exchanges, every calculator that simplifies fractions.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig03_euclid_staircase.png}
\caption{A step-by-step "descent staircase" diagram for the Euclidean algorithm applied to $(17, 5)$.}
\end{figure}
...
The alternating signs of the determinants produce the alternating signs in the convergents of a continued fraction\index{continued fractions} --- the numerators overshoot and undershoot the true value in strict alternation, a pattern that Bombelli noticed in 1572 and Wallis formalized in 1655. The Euclidean algorithm, continued fractions, and $2 \times 2$ matrices are three faces of the same crystal. We are about to discover a fourth.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig04_tree_vs_euclid.png}
\caption{A split-screen diagram.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig05_complexity_graph.png}
\caption{A graph with $N$ on the horizontal axis (logarithmic scale) and "number of steps" on the vertical axis.}
\end{figure}


The drama of integer factorization --- the problem that secures every online bank transaction, every encrypted email, every digital signature --- rests on this gap between $\sqrt{N}$ and the sub-exponential algorithms that can actually crack large semiprimes. We will explore those algorithms in later chapters. For now, the lesson is clear: the Berggren tree\index{Berggren tree} is beautiful, but it is a two-dimensional creature, and two dimensions are not enough.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig06_gauss_reduction.png}
\caption{A two-dimensional integer lattice with basis vectors $\mathbf{v}\_1$ and $\mathbf{v}\_2$ drawn as arrows from the origin.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig07_2d_barrier.png}
\caption{A dramatic conceptual image.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig08_lll_bargain.png}
\caption{A three-dimensional lattice rendered in perspective, with a cloud of lattice points filling a cube.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig09_quadruple_lattice.png}
\caption{A three-dimensional coordinate system with axes labeled $x$, $y$, $z$.}
\end{figure}


We call this the \textit{quadruple lattice} --- a nod to the quadratic form $x^2 + y^2 + z^2$ and to the algebraic structure it encodes. (The name also echoes the tradition of \textit{quadruple} representations in the theory of sums of squares, which stretches back through Jacobi and Lagrange to Fermat\index{Fermat, Pierre de} himself.)
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter2_fig10_concept_map.png}
\caption{A full-page "concept map" showing the logical flow of the chapter.}
\end{figure}
