...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig01_rectangle_dissection.png}
\caption{A $13 \times 5$ rectangle subdivided into unit squares.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig02_gaussian_lattice.png}
\caption{The Gaussian integer lattice $\mathbb{Z}[i]$ in the complex plane.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig03_triple_factory.png}
\caption{A whimsical "Triple Factory" conveyor belt diagram.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig04_multiplication_table.png}
\caption{A multiplication table of small Pythagorean triples.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig05_euler_xray.png}
\caption{"Euler's X-Ray Machine." A large number $N = 1105$ sits inside an ornate picture frame.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig06_euler_portrait.png}
...
We met this quadratic form in Chapter 1 --- the \textit{Lorentz\index{Lorentz, Hendrik} form}. A Pythagorean triple is nothing more than an integer point where $Q$ vanishes. The surface $Q = 0$ is a cone in three-dimensional space, and the Pythagorean triples\index{Pythagorean triple} are the lattice points scattered across its surface like stars on a celestial sphere.

In Chapter 1 we also met the three Berggren\index{Berggren, B.} transformations --- recipes that take one Pythagorean triple and manufacture three new ones. Let me write them out as explicit rules, so you can compute with them by hand:
...
Every cross-term cancels. Every coefficient conspires. What remains is exactly $Q(a, b, c)$ --- unchanged, invariant, preserved. If the input lives on the cone $Q = 0$, the output lives on the cone too. The Berggren transformations are \textit{discrete Lorentz transformations} --- integer symmetries of the same quadratic form\index{quadratic form} that governs the geometry of spacetime.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig07_pythagorean_cone.png}
\caption{A three-dimensional perspective drawing of the cone $x^2 + y^2 = z^2$ rising upward from the origin.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig08_light_cones.png}
\caption{A side-by-side comparison.}
\end{figure}


Physicists call this type of symmetry \textit{Lorentz invariance}, and it is one of the cornerstones of modern physics. The Berggren transformations are, in a precise sense, "quantized" Lorentz boosts --- discrete jumps rather than continuous rotations, but obeying the same fundamental constraint. The ancient theorem of Pythagoras and the modern theory of spacetime\index{spacetime} are branches of the same mathematical tree.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig09_berggren_tree.png}
\caption{A full ternary tree, three levels deep.}
\end{figure}


The history of this tree is a case study in mathematical rediscovery. B. Berggren, a Swedish schoolteacher, published the result in 1934 in a Swedish-language journal of such limited circulation that the mathematical world scarcely noticed. His paper proved that the three transformations generate every primitive triple from the seed $(3, 4, 5)$ --- a complete, constructive enumeration of an infinite set hidden in the most classical corner of number theory. Decades passed. In 1963, F. J. M. Barning rediscovered the tree independently. In 1970, A. Hall did it again. Today the Berggren tree\index{Berggren tree} is a staple of recreational mathematics and computational number theory, but its original discoverer, as far as anyone knows, never learned how famous his tree would become.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig10_tree_sieve.png}
\caption{A stylized tree with thick trunk and spreading branches.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig11_divisor_arcs.png}
\caption{A number line showing $N^2 = 31861^2$ with tick marks at selected divisor pairs.}
\end{figure}
...
\textbf{Road 2 --- Gauss\index{Gauss, Carl Friedrich}ian Composition.} The Brahmagupta–Fibonacci identity lets you \textit{manufacture} new sum-of-squares representations by composing existing ones, amplifying partial information into a full factorization.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig12_three_roads_map.png}
\caption{A stylized map drawn in the manner of a medieval cartographer.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig13_exponential_climb.png}
\caption{A vertical logarithmic scale running from $10^0$ at the bottom to $10^{20}$ at the top.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig14_flower_diagram.png}
\caption{A "flower" diagram.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig15_factoring_toolkit.png}
\caption{A "Pythagorean Factoring Toolkit" infographic divided into three columns.}
\end{figure}


The grand irony is that these methods, beautiful as they are, do not (as far as anyone knows) yield a fast algorithm for factoring large semiprimes\index{semiprime}. The tree sieve reaches any target in $\sim \log_3 N$ generations, but finding the right branch requires searching an exponentially large space. Euler's method needs two representations, and finding even one is as hard as factoring itself. Gaussian composition can amplify partial information, but the partial information must come from somewhere.

And yet the ideas in this chapter are not idle curiosities. They are the \textit{ancestors} of the methods that \textit{do} work in practice. The quadratic sieve\index{quadratic sieve} and the number field sieve --- the fastest known classical factoring algorithms --- are direct descendants of the idea that smooth relations, combined algebraically, can reveal factors. And Shor\index{Shor, Peter}'s quantum algorithm, which would factor large semiprimes in polynomial time on a quantum computer, exploits the same multiplicative structure that Brahmagupta discovered fourteen centuries ago.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{images/chapter4_fig16_road_ahead.png}
\caption{A winding road receding into a misty distance.}
\end{figure}
