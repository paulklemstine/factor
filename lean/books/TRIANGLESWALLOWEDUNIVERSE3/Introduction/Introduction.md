...
## Section 1: The Puzzle on the Bathroom Tiles
...
[ILLUSTRATION: A large, beautifully rendered depiction of a right triangle with legs labeled $a = 3$ and $b = 4$ and hypotenuse $c = 5$. On each side of the triangle, a perfect square grid is constructed outward: a $3 \times 3$ grid of unit squares on the shorter leg (shaded in sky blue), a $4 \times 4$ grid on the longer leg (shaded in warm gold), and a $5 \times 5$ grid on the hypotenuse (shaded in soft coral). The $9$ blue tiles and $16$ gold tiles are visually rearranged — an arrow or "explosion" diagram shows them fitting snugly into the $25$-tile coral square, confirming $9 + 16 = 25$. Beside the triangle, a small photograph-style rendering of the Plimpton 322 clay tablet, roughly palm-sized, covered in cuneiform numerals arranged in neat columns, with a caption reading "Babylon, c. 1800 BCE."]
...
[ILLUSTRATION: A table of the first sixteen primitive Pythagorean triples, organized in four columns. Column 1: the triple $(a, b, c)$ with $a$ odd and $b$ even. Column 2: the Euclid parameters $(m, n)$ that generate each triple. Column 3: the value of $c - b$, highlighted in a pale yellow background. Column 4: the value of $c - a$, highlighted in pale green. The triples listed are: $(3,4,5)$, $(5,12,13)$, $(8,15,17)$, $(7,24,25)$, $(20,21,29)$, $(9,40,41)$, $(12,35,37)$, $(11,60,61)$, $(13,84,85)$, $(28,45,53)$, $(33,56,65)$, $(36,77,85)$, $(20,99,101)$, $(48,55,73)$, $(39,80,89)$, $(65,72,97)$. At the bottom, a note in italics reads: "Notice anything about the $c - b$ column?" The values in that column are $1, 1, 2, 1, 8, 1, 2, 1, 1, 8, 9, 8, 2, 18, 9, 25$ — a sequence that repays close inspection.]

Since we can choose $m$ and $n$ to be as large as we please, the formula generates infinitely many triples. The bathroom tiles never run out. But a new question immediately surfaces: does this formula generate *all* Pythagorean triples, or only some of them? And among the triples it generates, are there repetitions — different choices of $(m, n)$ that happen to produce the same triple?
...
### Primitive vs. Imprimitive: The Genealogy of Triples

Call a Pythagorean triple *primitive* if the three numbers $a$, $b$, and $c$ share no common factor greater than $1$. The triple $(3, 4, 5)$ is primitive: no integer except $1$ divides all three. But the triple $(6, 8, 10)$ is *not* primitive — it is simply $(3, 4, 5)$ multiplied through by $2$. The triple $(9, 12, 15)$ is $(3, 4, 5)$ multiplied by $3$. In fact, every Pythagorean triple is either primitive or a positive integer multiple of a primitive one. (If $d$ divides $a$, $b$, and $c$, then $a/d$, $b/d$, $c/d$ is still Pythagorean — just divide the equation $a^2 + b^2 = c^2$ through by $d^2$.)

This means that to understand *all* Pythagorean triples, it suffices to understand the *primitive* ones. Everything else is just rescaling. And here is the beautiful theorem, attributed to Euclid but refined over the centuries:

> **Euclid's Classification.** Every primitive Pythagorean triple $(a, b, c)$ with $a$ odd and $b$ even is given by
> $$a = m^2 - n^2, \qquad b = 2mn, \qquad c = m^2 + n^2$$
...
With these conditions in place, the Euclid parametrization is a *perfect census*. Every primitive triple is counted exactly once. No triple is missed. No triple is counted twice. It is, in mathematical language, a *bijection* between valid parameter pairs and primitive triples.

---

### The Question That Drives This Book

And yet — a census is not a *family tree*.

A census lists every citizen of a country, alphabetically or by district. A family tree does something more: it shows you who begat whom. It reveals *structure*. It tells you that $(5, 12, 13)$ is not merely an item on a list but the *offspring* of $(3, 4, 5)$, produced by a specific transformation. It tells you that $(3, 4, 5)$ is the *ancestor* of every primitive triple — the root from which the entire infinite family descends.

Is there such a tree? Is there a way to organize every primitive Pythagorean triple into a branching structure — a tree in the mathematician's sense, with a single root and no cycles — so that each triple appears exactly once, and each triple (except the root) has a unique *parent* from which it was born?

The answer is yes. The tree exists, it was discovered in 1934 by a Swedish mathematician named Berggren, and it is one of the most beautiful objects in all of number theory. But to appreciate *why* it is beautiful — why it is far more than a clever filing system — we will need to see what it is made of. We will need to see the three transformations that generate it, the strange geometry that governs it, and the deep, unexpected connection it has to a branch of physics that would not be invented for another thirty years after Berggren wrote his paper.
...
[ILLUSTRATION: A whimsical, full-page scene depicting a person sitting on the edge of a bathtub, staring at a bathroom floor tiled with white unit squares. On the floor, three regions of tiles are outlined in colored chalk: a $3 \times 3$ blue square, a $4 \times 4$ gold square, and a $5 \times 5$ coral square, arranged to form the classic Pythagorean theorem diagram around a right triangle drawn in the grout lines. A thought bubble above the person's head contains a branching tree with the labels $(3,4,5)$ at the root, branching into three children, each branching into three more — fading into a fractal haze. The bathtub is overflowing slightly, unnoticed. On the windowsill behind, a small clay tablet (Plimpton 322) leans against a rubber duck.]
