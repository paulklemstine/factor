...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig01_locksmith_tree.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig02_euclid_staircase.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig03_step_count_plot.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig04_danger_zone.png}
\caption*{Danger Zone}
\end{figure}

\index{The Lazy Locksmith's Wager}
...
His plan is elegant. He knows, from the previous chapters, that every primitive Pythagorean triple sits as a node in an infinite ternary tree, and that the legs of these triples can betray the factors of composite numbers. He will start at some specially chosen triple high in the tree and descend, level by level, computing the greatest common divisor of each leg with $N$ until a nontrivial factor drops into his lap.\index{Pythagorean triple}
...
\begin{quote}
\itshape
\textit{"No matter how clever your descent is,"} she says, \textit{"you will never crack the lock faster than about $\sqrt{N}$ steps. And in the worst case, you will need every single one of them."}\index{descent}
\end{quote}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig05_nearly_square.png}
\caption*{Nearly Square}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig06_gcd_node.png}
\caption*{Gcd Node}
\end{figure}
...
Twelve steps! Every quotient is $1$ except the very last --- the algorithm spirals inward with agonizing slowness, like a nautilus shell winding toward its center. And the GCD, after all that labor, is $1$: the two numbers are coprime.\index{greatest common divisor}

This is no accident. The numbers $377$ and $233$ are consecutive Fibonacci numbers ($F_{14}$ and $F_{13}$), and consecutive Fibonacci numbers are the \textit{worst possible inputs} for the Euclidean algorithm. The reason is that the Fibonacci recurrence $F_{n+1} = F_n + F_{n-1}$ is precisely the recurrence that makes the quotient $1$ at every stage, so no large chunk is ever subtracted away. The algorithm is forced to take the maximum number of steps.\index{Euclidean algorithm}
...
Why? Because $\gcd(a, b)$ divides both $a$ and $b$, and no divisor of a positive number can exceed that number. This innocent inequality will reappear when we count the nodes on the descent path: the GCD at each node is a "measuring rod" whose length never exceeds the shorter of the two numbers being compared.\index{Shor's algorithm}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig07_descent_clocks.png}
\caption*{Descent Clocks}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig08_multiplication_rect.png}
\caption*{Multiplication Rect}
\end{figure}

\index{The Balanced Semiprime --- A Wolf in Sheep's Clothing}
...
Numbers like $10{,}403$ are called \textit{balanced semiprimes}: products $N = p \cdot q$ of two primes that are roughly equal in size. They are the hardest nuts for any factoring method to crack, and the reason is geometric.\index{semiprime}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig09_complexity_vise.png}
\caption*{Complexity Vise}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig10_three_way_race.png}
\caption*{Three Way Race}
\end{figure}

\index{The Cost of a Handshake --- GCD at Every Node}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig11_comparison_table.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig12_dimensional_escape.png}
\caption*{Dimensional Escape}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig13_factoring_curves.png}
\caption*{Factoring Curves}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig14_quantum_descent.png}
\caption*{Quantum Descent}
\end{figure}

\index{The Floor Beneath the Floor --- Why You Can't Go Faster}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter8/images/fig15_full_recap.png}
\caption*{Full Recap}
\end{figure}
...
Three contestants enter a factoring race. Contestant A, \textit{Trial Division}, is the oldest and simplest method known: start at $2$ and test every successive integer (or prime) to see if it divides $N$. Contestant B, \textit{Fermat's Method}, is cleverer: start at $\lceil \sqrt{N} \rceil$ and search for a value $x$ such that $x^2 - N$ is a perfect square, for then $N = x^2 - y^2 = (x - y)(x + y)$, and the factors are $x - y$ and $x + y$. Contestant C, the \textit{Pythagorean Tree}, is our locksmith's method.\index{Fermat, Pierre de}
...
For the balanced semiprimes that matter most in cryptography --- the case $p \approx q \approx \sqrt{N}$ --- all three methods converge to the same speed:\index{cryptography}
