...
def berggren_M? : Matrix (Fin 2) (Fin 2) Int := !![2, -1; 1, 0]
def berggren_M? : Matrix (Fin 2) (Fin 2) Int := !![1, 2; 0, 1]
def berggren_M?_inv : Matrix (Fin 2) (Fin 2) Int := !![0, 1; -1, 2]
def berggren_M?_inv : Matrix (Fin 2) (Fin 2) Int := !![1, -2; 0, 1]

/-- M? has determinant 1 (lives in SL(2,Int)). -/
...
theorem berggren_M?_mul_inv :
    berggren_M? * berggren_M?_inv = (1 : Matrix (Fin 2) (Fin 2) Int) := by
...
of quotients as the Euclidean algorithm. M?^(-1) reduces m by 2n (preserving n),
and M?^(-1) implements the swap step. Together they execute the continued
...
/-- The quadruple lattice condition: x^2 + y^2 + z^2 === 0 (mod N^2). -/
