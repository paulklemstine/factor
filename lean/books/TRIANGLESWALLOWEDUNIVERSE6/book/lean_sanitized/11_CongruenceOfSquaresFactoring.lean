...
reduces to finding x, y such that x^2 === y^2 (mod n) with x ? ?y (mod n),
...
/-
If `n` divides `x^2 - y^2` but does not divide `x - y` or `x + y`,
...
The product of gcd(x-y, n) and gcd(x+y, n) is divisible by n
    when n divides x^2 - y^2. This is essential: after finding one factor
    via gcd(x-y, n), the cofactor n / gcd(x-y, n) divides gcd(x+y, n).
-/
theorem congruence_of_squares_cofactor
    {n x y : Int} (hn : 1 < n)
    (hcong : (n : Int) | x ^ 2 - y ^ 2) :
    (n : Int) | up(Int.gcd (x - y) n) * up(Int.gcd (x + y) n) := by
  grind +suggestions
...
-/
theorem gcd_sub_dvd_n (x y n : Int) : up(Int.gcd (x - y) n) | n := by
  exact Int.gcd_dvd_right _ _

/-
If n divides x^2 - y^2, then gcd(x - y, n) * gcd(x + y, n) is a
    multiple of n that is bounded by n^2.
-/
