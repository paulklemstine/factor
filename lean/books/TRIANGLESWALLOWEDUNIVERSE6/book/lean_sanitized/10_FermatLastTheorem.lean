...
Actually, Fermat proved the stronger result: a? + b? = c^2 has no
positive integer solutions. This immediately implies FLT for n = 4
since a? + b? = c? would give a? + b? = (c^2)^2.

**Proof sketch (infinite descent):**
Suppose a? + b? = c^2 with a, b, c > 0 and c minimal.
...
/-
PROBLEM
The stronger form: a? + b? != c^2 for positive integers.
...
FLT for n = 3: a^3 + b^3 != c^3 for positive integers.
    First proved by Euler (1770), using factorization in Int[?]
...
Fermat likely attempted to prove FLT by factoring in Int[??], the ring
of integers of the cyclotomic field Rat(??), where ?? = e^{2?i/n}.

The argument would go:
1. From a? + b? = c?, factor the left side:
   (a + b)(a + ?b)(a + ?^2b)***(a + ??^(-1)b) = c?
2. If these factors are pairwise coprime and Int[??] has unique
   factorization, then each factor must be an n-th power.
3. Derive a contradiction.

**The flaw:** Int[??] does NOT always have unique factorization!
It fails for n = 23 (discovered by Kummer, 1847).

Kummer saved the approach for "regular primes" (primes p where p
does not divide the class number of Rat(??)), proving FLT for all
