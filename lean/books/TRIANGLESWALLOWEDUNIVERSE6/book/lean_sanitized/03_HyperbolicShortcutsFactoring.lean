...
1. **Difference-of-squares from Pythagorean triples**: Every Pythagorean triple
   `a^2 + b^2 = c^2` yields a factorization `(c-b)(c+b) = a^2`.

2. **Berggren matrices as Lorentz isometries**: All three generators preserve the
   quadratic form Q(a,b,c) = a^2 + b^2 - c^2, placing them in O(2,1;Int).
...
group O(2,1;Int). Each matrix preserves the quadratic form Q(a,b,c) = a^2 + b^2 - c^2.

A "hyperbolic shortcut" is a composite matrix B_{i?} * B_{i?} * ? * B_{i?}
...
/-- If a^2 + b^2 = c^2 with a > 0 and c > 0, then b < c. -/
...
def B? : Matrix (Fin 3) (Fin 3) Int := !![1, -2, 2; 2, -1, 2; 2, -2, 3]
...
/-- B? preserves the Lorentz form: B?? Q B? = Q. -/
theorem B?_lorentz : B?? * Q * B? = Q := by native_decide
...
/-- det(B?) = 1: B? is in SO?(2,1;Int). -/
theorem det_B? : Matrix.det B? = 1 := by native_decide

/-- det(B?) = -1: B? is in O(2,1;Int) \ SO(2,1;Int). -/
theorem det_B? : Matrix.det B? = -1 := by native_decide
...
theorem B?_preserves_pyth (a b c : Int) (h : a ^ 2 + b ^ 2 = c ^ 2) :
...
theorem B?_hyp_increases (a b c : Int) (ha : 0 < a) (hb : 0 < b) (hc : 0 < c) :
    c < 2*a + 2*b + 3*c := by linarith

/-- The hypotenuse strictly increases under B? when a^2 + b^2 = c^2, a > 0, c > 0. -/
theorem B?_hyp_increases (a b c : Int) (ha : 0 < a) (hc : 0 < c)
    (h : a ^ 2 + b ^ 2 = c ^ 2) :
    c < 2*a - 2*b + 3*c := by nlinarith [sq_nonneg (c - b)]

/-- The hypotenuse strictly increases under B? when a^2 + b^2 = c^2, b > 0, c > 0. -/
theorem B?_hyp_increases (a b c : Int) (hb : 0 < b) (hc : 0 < c)
    (h : a ^ 2 + b ^ 2 = c ^ 2) :
    c < -2*a + 2*b + 3*c := by nlinarith [sq_nonneg (c - a)]

/-- B?-branch hypotenuse is at least 3c (geometric growth). -/
theorem B?_hyp_triple_growth (a b c : Int) (ha : 0 < a) (hb : 0 < b) :
...
theorem B?_iter1_hyp :
    (B? *? (![3, 4, 5] : Fin 3 -> Int)) 2 = 29 := by native_decide

/-- B? applied twice gives hypotenuse 169. -/
theorem B?_iter2_hyp :
    (B? *? (B? *? (![3, 4, 5] : Fin 3 -> Int))) 2 = 169 := by native_decide

/-- B? applied three times gives hypotenuse 985. -/
theorem B?_iter3_hyp :
    (B? *? (B? *? (B? *? (![3, 4, 5] : Fin 3 -> Int)))) 2 = 985 := by native_decide
...
/-- Inverse of B?: computed as Q * B?? * Q. -/
def B?_inv : Matrix (Fin 3) (Fin 3) Int := !![1, 2, -2; -2, -1, 2; -2, -2, 3]
...
theorem B?_inv_formula : B?_inv = Q * B?? * Q := by native_decide
...
/-- The difference-of-squares from (21, 20, 29): (29-20)(29+20) = 9*49 = 441 = 21^2. -/
...
theorem B?_preserves_lorentzQ (a b c : Int) :
