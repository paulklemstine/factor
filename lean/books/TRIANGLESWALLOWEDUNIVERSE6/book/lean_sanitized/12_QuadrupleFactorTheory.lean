...
/-
**Sum-of-Squares Collision Principle**: If N = a?^2 + b?^2 + c?^2 = a?^2 + b?^2 + c?^2
    with different c values, then (c?^2-c?^2) = (a?^2-a?^2) + (b?^2-b?^2).
-/
theorem collision_factor_extraction (a? b? c? a? b? c? d : Int)
    (h? : a? ^ 2 + b? ^ 2 + c? ^ 2 = d ^ 2)
    (h? : a? ^ 2 + b? ^ 2 + c? ^ 2 = d ^ 2) :
...
theorem collision_difference_product (a? b? c? a? b? c? d : Int)
    (h? : a? ^ 2 + b? ^ 2 + c? ^ 2 = d ^ 2)
    (h? : a? ^ 2 + b? ^ 2 + c? ^ 2 = d ^ 2) :
