...
theorem gcd_cross_sign (c? c? d g : Int)
    (h1 : g | (d - c?)) (h2 : g | (d + c?)) :
    g | (c? + c?) := by
  have : c? + c? = (d + c?) - (d - c?) := by ring
...
theorem cascade_transitive (c? c? d g : Int)
    (h1 : g | (d - c?)) (h12 : g | (c? - c?)) :
    g | (d - c?) := by
...
theorem double_cascade (c? c? c? d p : Int)
...
theorem shared_hyp_channel (a? b? c? a? b? c? d : Int)
    (h? : a?^2 + b?^2 + c?^2 = d^2) (h? : a?^2 + b?^2 + c?^2 = d^2) :
...
/-- If p | d and p | c?, then p^2 | (a?^2+b?^2). -/
theorem shared_factor_asymmetry (a? b? c? d p : Int)
...
def repDist (a? b? c? a? b? c? : Int) : Int :=
  (a? - a?)^2 + (b? - b?)^2 + (c? - c?)^2

theorem repDist_eq (a? b? c? a? b? c? d : Int)
    (h? : a?^2 + b?^2 + c?^2 = d^2) (h? : a?^2 + b?^2 + c?^2 = d^2) :
    repDist a? b? c? a? b? c? = 2 * d^2 - 2 * (a?*a? + b?*b? + c?*c?) := by
  unfold repDist; nlinarith

theorem repDist_nonneg (a? b? c? a? b? c? : Int) :
    0 <= repDist a? b? c? a? b? c? := by unfold repDist; positivity

theorem repDist_antipodal (a? b? c? d : Int) (h : a?^2 + b?^2 + c?^2 = d^2) :
    repDist a? b? c? (-a?) (-b?) (-c?) = 4 * d^2 := by
  unfold repDist; nlinarith

theorem repDist_zero_iff (a? b? c? a? b? c? : Int) :
...
theorem inner_product_bound (a? b? c? a? b? c? d : Int)
    (h? : a?^2 + b?^2 + c?^2 = d^2) (h? : a?^2 + b?^2 + c?^2 = d^2) :
...
theorem fingerprint_diff_zero (a? b? c? a? b? c? d : Int)
    (h? : a?^2 + b?^2 + c?^2 = d^2) (h? : a?^2 + b?^2 + c?^2 = d^2) :
...
-- Q = (6, 9, 18, 21)? 36+81+324 = 441 = 21^2 [ok]
