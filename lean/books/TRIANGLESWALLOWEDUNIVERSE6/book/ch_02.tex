...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig01_factory_tree.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig02_lattice_shear.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig03_euclid_staircase.png}
...
\begin{quote}
\itshape
\textit{Can these two machines --- and nothing else --- manufacture every primitive Pythagorean triple that will ever exist?}\index{Pythagorean triple!primitive}
\end{quote}
...
Already the tree is branching. Two children at each node, four grandchildren, eight great-grandchildren, and so on, the family growing without bound. Convert each $(m, n)$ pair back to a Pythagorean triple using Euclid's formulas --- $a = m^2 - n^2$, $b = 2mn$, $c = m^2 + n^2$ --- and you have a genealogical chart of right triangles.\index{Euclid}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig05_complexity_graph.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig06_gauss_reduction.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig07_2d_barrier.png}
\caption*{2D Barrier}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig08_lll_bargain.png}
\caption*{Lll Bargain}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig09_quadruple_lattice.png}
\caption*{Quadruple Lattice}
\end{figure}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{../Chapter2/images/fig10_concept_map.png}
\caption*{Concept Map}
\end{figure}

\index{The Astonishing Coincidence}
...
Suppose $N$ is the product of two secret prime numbers, $p$ and $q$, and we are trying to discover them. One approach --- inspired by the factoring bridge from Chapter 1 --- is to search the Berggren tree for a triple whose entries reveal a factor of $N$. The tree descent gives us a systematic way to do this: start at a carefully chosen node and trace a path back to the root, checking at each step whether the current triple's parameters share a common factor with $N$.\index{factoring}
...
Call $N = p \cdot q$ a \textit{balanced semiprime}, where $2 \le p \le q$. (The word "balanced" is a slight misnomer --- we do not require $p$ and $q$ to be close, only that $p$ is the smaller factor.) Then:\index{semiprime}
...
\index{The Wall at Two Dimensions}
...
4. Therefore, \textit{no trick within the two-dimensional framework can beat $\Theta(\sqrt{N})$ for factoring}.
...
\index{Through the Looking Glass: Higher Dimensions}
...
We call this the \textit{quadruple lattice} --- a nod to the quadratic form $x^2 + y^2 + z^2$ and to the algebraic structure it encodes. (The name also echoes the tradition of \textit{quadruple} representations in the theory of sums of squares, which stretches back through Jacobi and Lagrange to Fermat himself.)\index{Fermat, Pierre de}
