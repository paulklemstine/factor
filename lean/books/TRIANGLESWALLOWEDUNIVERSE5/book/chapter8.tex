\section{\texorpdfstring{\emph{How Hard Is It to Factor by Climbing a Tree?}}{How Hard Is It to Factor by Climbing a Tree?}}\label{ch8-how-hard-is-it-to-factor-by-climbing-a-tree}



\section{The Lazy Locksmith's Wager}\label{ch8-the-lazy-locksmiths-wager}
...
\begin{quote}
\emph{``No matter how clever your descent is,''} she says, \emph{``you will never crack the lock faster than about \(\sqrt{N}\) steps. And in the worst case, you will need every single one of them.''}
\end{quote}
...
The answer to Elsa's wager is the subject of this entire chapter, and I will not keep you in suspense: \textbf{she wins}. The Pythagorean tree factoring method requires \(\Theta(\sqrt{N})\) arithmetic operations --- a quantity that is simultaneously an upper bound (you never need more) and a lower bound (you sometimes need every last one). In the language of computational complexity, the cost is \emph{tight}. The tree is beautiful, the geometry is deep, but the speed limit is real.

To appreciate why, we must understand three things: how long the descent can possibly be, how much work each step costs, and why no shortcut exists. The journey will take us from Euclid's ancient algorithm through the arithmetic of balanced \index{semiprime}semiprimes to the tantalizing escape routes that higher dimensions offer. Let us begin with the oldest algorithm in mathematics.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig01_locksmith_tree.png}
\end{figure}




\section{Euclid's Staircase --- How Long Is the Climb?}\label{ch8-euclids-staircase-how-long-is-the-climb}
...
\[377 = 1 \times 233 + 144\] \[233 = 1 \times 144 + 89\] \[144 = 1 \times 89 + 55\] \[89 = 1 \times 55 + 34\] \[55 = 1 \times 34 + 21\] \[34 = 1 \times 21 + 13\] \[21 = 1 \times 13 + 8\] \[13 = 1 \times 8 + 5\] \[8 = 1 \times 5 + 3\] \[5 = 1 \times 3 + 2\] \[3 = 1 \times 2 + 1\] \[2 = 2 \times 1 + 0.\]
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig02_euclid_staircase.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig03_step_count_plot.png}
\end{figure}
...
\section{The Balanced Semiprime --- A Wolf in Sheep's Clothing}\label{ch8-the-balanced-semiprime-a-wolf-in-sheeps-clothing}
...
Numbers like \(10{,}403\) are called \emph{balanced semiprimes}: products \(N = p \cdot q\) of two primes that are roughly equal in size. They are the hardest nuts for any factoring method to crack, and the reason is geometric.
...
The smaller factor of any semiprime never exceeds the square root of the product. This is the \emph{fundamental bound} --- it tells us that any search for the smaller factor need not look beyond \(\sqrt{N}\).
...
This means \(m\) is always strictly less than the hypotenuse --- a guarantee of \emph{progress}. As we descend the tree, the parameter \(m\) decreases at every step, and since \(m \le \sqrt{c} \le \sqrt{N}\), the descent visits at most \(O(\sqrt{N})\) nodes. This is the depth bound: the tree is deep, but not infinitely so.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig04_danger_zone.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig05_nearly_square.png}
\end{figure}




\section{The Cost of a Handshake --- GCD at Every Node}\label{ch8-the-cost-of-a-handshake-gcd-at-every-node}
...
Or, if we count in the more generous currency of \emph{arithmetic operations} --- where a single division of two numbers counts as one step, regardless of how many digits they have --- then each GCD costs \(O(\log N)\) arithmetic operations.
...
The Euclidean algorithm, incidentally, has a plausible claim to being the oldest nontrivial algorithm in existence. Its earliest appearance is in Book VII of Euclid's \emph{Elements}, composed around 300 BCE in Alexandria, but it was almost certainly known earlier --- perhaps to the Pythagoreans themselves, perhaps to Babylonian scribes working with reciprocal tables a millennium before Euclid. Donald Knuth, in \emph{The Art of Computer Programming}, calls it ``the granddaddy of all algorithms'' and devotes a small treatise to its analysis. What Euclid could not have imagined is that his simple procedure of repeated subtraction would one day be the inner loop of a factoring algorithm, executed billions of times per second inside machines that would have seemed to him indistinguishable from magic.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig06_gcd_node.png}
\end{figure}




\section{Multiplying the Bill --- The Total Cost of the Descent}\label{ch8-multiplying-the-bill-the-total-cost-of-the-descent}
...
\textbf{Elsa's Theorem (Upper Bound).} \emph{For a balanced semiprime \(N = p \cdot q\) with \(2 \le p \le q\), the Pythagorean tree factoring method finds a factor of \(N\) in at most}

\[O\!\left(\sqrt{N} \cdot \log N\right) \text{ bit operations,}\]

\emph{or equivalently, \(O(\sqrt{N})\) arithmetic operations.}
...
This is the \emph{upper bound} --- the promise that Laszlo's descent will always terminate within this budget. But is it tight? Could a cleverer locksmith do better? The next section will show that the answer is no.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig07_descent_clocks.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig08_multiplication_rect.png}
\end{figure}




\section{The Floor Beneath the Floor --- Why You Can't Go Faster}\label{ch8-the-floor-beneath-the-floor-why-you-cant-go-faster}

Imagine you are told that a number \(N\) is the product of two primes, one of which is at least \(2\). Can you find a factor without doing \emph{any} work? Clearly not. You must do at least \emph{something}. But how much is ``something''?
...
\[\Omega(\sqrt{N}) \le \text{Cost} \le O(\sqrt{N})\] \[\implies \text{Cost} = \Theta(\sqrt{N}).\]

The locksmith's tree, for all its geometric elegance, is no faster than plodding through every candidate factor from \(2\) to \(\sqrt{N}\). The \(\sqrt{N}\) barrier is real, and within the two-dimensional world of \index{Pythagorean triple}Pythagorean triples, it is unbreakable.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig09_complexity_vise.png}
\end{figure}




\section{Old Rivals --- Trial Division and Fermat's Method}\label{ch8-old-rivals-trial-division-and-fermats-method}

Three contestants enter a factoring race. Contestant A, \emph{Trial Division}, is the oldest and simplest method known: start at \(2\) and test every successive integer (or prime) to see if it divides \(N\). Contestant B, \emph{Fermat's Method}, is cleverer: start at \(\lceil \sqrt{N} \rceil\) and search for a value \(x\) such that \(x^2 - N\) is a perfect square, for then \(N = x^2 - y^2 = (x - y)(x + y)\), and the factors are \(x - y\) and \(x + y\). Contestant C, the \emph{Pythagorean Tree}, is our locksmith's method.

Who wins?

The surprising answer: \emph{it depends on the terrain.}
...
This convergence is not a coincidence. It reflects a deep truth about the problem: \emph{in two dimensions, \(\sqrt{N}\) is the natural scale of the factoring problem.} Whether you walk the number line from left to right (trial division), search outward from the center (Fermat), or descend a tree rooted in Pythagorean geometry, you are always navigating a two-dimensional landscape, and the \(\sqrt{N}\) barrier is the horizon of that landscape. To go faster, you must escape into higher dimensions.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig10_three_way_race.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig11_comparison_table.png}
\end{figure}




\section{Shattering the Ceiling --- The Escape to Higher Dimensions}\label{ch8-shattering-the-ceiling-the-escape-to-higher-dimensions}
...
As \(d\) grows, the shortest vector shrinks, and with it the cost of the ``descent.'' This is the dimensional escape. The price of admission is that working in \(d\) dimensions costs more per step --- the lattice arithmetic is more expensive, and finding short vectors in high-dimensional lattices is itself a hard problem. But the trade-off is favorable: the savings from shorter vectors outpace the overhead of higher-dimensional arithmetic. The optimal balance between these two pressures leads to \emph{sub-exponential} factoring algorithms --- methods that are faster than any polynomial in \(\sqrt{N}\) but slower than any fixed polynomial in \(\log N\).

The first practical realization of this idea was the \textbf{LLL algorithm}, published in 1982 by Arjen Lenstra, Hendrik Lenstra, and László Lovász. LLL finds \emph{approximately} shortest vectors in \(d\)-dimensional lattices with an approximation factor of \(2^{(d-1)/2}\), in polynomial time. It was a revolution. Before LLL, \index{lattice reduction}lattice reduction was an art; after LLL, it was a pushbutton technology. Hendrik Lenstra used it to factor a 60-digit number on an Apple II --- a machine with 48 kilobytes of RAM and a processor slower than a modern wristwatch.

The story of that factorization deserves a brief retelling. It was 1983, and the factoring record for general-purpose algorithms stood at about 50 digits. Lenstra's \index{elliptic curves}elliptic curve method, combined with LLL lattice reduction, cracked a 60-digit number in a computation that ran for several hours on hardware that would today be outperformed by a pocket calculator. The mathematical community was stunned --- not by the specific number factored, but by the \emph{method}. Lattice reduction opened a door that trial division, Fermat, and the Pythagorean tree could never unlock.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig13_factoring_curves.png}
\end{figure}




\section{The Quantum Shortcut --- Grover and the Fourth Root}\label{ch8-the-quantum-shortcut-grover-and-the-fourth-root}

Suppose the locksmith could try multiple keys \emph{simultaneously}, in quantum superposition. The tree's descent path is deterministic --- at each level, only one branch is valid --- but the \emph{depth} at which the factor appears is unknown. Laszlo doesn't know how deep to go. Classically, he has no choice but to try level after level, from top to bottom, until the GCD check succeeds. But a quantum computer can search an unstructured space of \(M\) possibilities in only \(O(\sqrt{M})\) queries --- this is Grover's algorithm, proved by Lov Grover in 1996.
...
There is, however, a subtle catch. Grover's algorithm provides a speedup for \emph{unstructured search} --- searching a list of candidates when you have no information about which one is correct. But the tree descent is not entirely unstructured; it is sequential. Each level depends on the result of the previous level. Grover can help Laszlo guess the right \emph{depth} to start his descent, but it cannot help him skip steps within the descent itself. The determinism theorem from Chapter 7 --- the one-way corridor --- is a barrier that even quantum mechanics cannot circumvent.

The full picture is this:

\[\text{Classical: } \Theta(\sqrt{N}) \xrightarrow{\text{Grover}} O(N^{1/4}).\]

And there is a deeper question lurking beneath the surface: can higher-dimensional lattice methods, combined with quantum search, break below \(N^{1/4}\)? The answer, as of the time of this writing, is unknown. Shor's algorithm, of course, factors integers in polynomial time on a quantum computer --- but Shor does not use the Pythagorean tree at all. The question is whether the \emph{geometric} approach, the descent through Pythagorean space, can be enhanced by quantum mechanics to achieve polynomial time. This remains one of the beautiful open problems at the intersection of quantum computation and number theory.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig14_quantum_descent.png}
\end{figure}




\section{The Moral of the Tree --- What Complexity Tells Us About Structure}\label{ch8-the-moral-of-the-tree-what-complexity-tells-us-about-structure}
...
What does this coincidence \emph{mean}?

I believe it means something deep about the relationship between dimension and difficulty. The Pythagorean tree lives in the hyperbolic plane --- a two-dimensional space of constant negative curvature, as we saw in Chapter 5. The triples are lattice points on the light cone, and the Berggren matrices are elements of \(SO(2, 1, \mathbb{Z})\), the discrete \index{Lorentz group}Lorentz group in \((2+1)\) dimensions. The \(\sqrt{N}\) barrier is not an accident of the algorithm; it is a reflection of the \emph{geometry} of the space in which the algorithm operates. Two-dimensional hyperbolic space has a definite ``width,'' and that width determines the cost of traversal.

When we move to higher-dimensional lattices, we are not merely using cleverer algebra. We are stepping out of the hyperbolic plane into a higher-dimensional hyperbolic space where the lattice points are packed more densely, the shortest vectors are shorter, and the descent paths cut across dimensions that were previously invisible. The sub-exponential factoring algorithms --- the \index{quadratic sieve}quadratic sieve, the number field sieve --- are, in a sense, \emph{explorations of higher-dimensional light cones}. They find short vectors in high-dimensional lattices, and those short vectors correspond to factorizations.

There is one more thought I want to leave you with, and it is the kind of thought that keeps mathematicians awake at night. The Pythagorean tree is a discrete subgroup of the Lorentz group --- the group of symmetries of Einstein's spacetime. The \(\sqrt{N}\) barrier arises from the two-dimensionality of the hyperbolic plane. The escape to higher dimensions mimics the passage from \((2+1)\)-dimensional spacetime to \((d+1)\)-dimensional spacetime. If factoring is really a problem about the \emph{geometry of discrete symmetry groups}, then the difficulty of factoring is not merely a computational curiosity --- it is a statement about the \emph{structure of space itself}.

Is factoring hard because spacetime is low-dimensional? Would factoring be easy in a universe with more spatial dimensions? These questions sound like science fiction, but they are precisely the kind of questions that arise when you take the geometry of the Pythagorean tree seriously. The tree has told us something profound: \emph{the cost of descent depends on the curvature of the space you descend through.} And in our two-dimensional slice of the mathematical universe, the price of descent is exactly \(\sqrt{N}\) --- not a penny more, not a penny less.

Laszlo pays his debt to Elsa. The locksmith cannot beat the square root. But he gazes upward at the higher branches of the tree and wonders: \emph{what if the tree grew in more dimensions?}


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter8/images/fig15_full_recap.png}
\end{figure}




\emph{In the next chapter, we will ask a different question entirely. Instead of descending the tree to factor a single number, we will climb upward and ask: how are the factors distributed across the tree as a whole? The answer will involve a surprising connection to the Riemann zeta function --- and the deepest unsolved problem in mathematics.}
