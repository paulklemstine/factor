\section{\texorpdfstring{\emph{Why Quantum Shortcuts Aren't Where You'd Expect}}{Why Quantum Shortcuts Aren't Where You'd Expect}}\label{ch7-why-quantum-shortcuts-arent-where-youd-expect}



\section{The Forking Labyrinth}\label{ch7-the-forking-labyrinth}
...
\section{The Cancellation Trick}\label{ch7-the-cancellation-trick}
...
\section{A Parable of the Circular Library}\label{ch7-a-parable-of-the-circular-library}
...
\section{Searching for the Magic Depth}\label{ch7-searching-for-the-magic-depth}
...
\section{Balanced Semiprimes and the Fourth-Root Barrier}\label{ch7-balanced-semiprimes-and-the-fourth-root-barrier}

A cryptographer builds a lock from the product \(N = p \times q\) of two secret primes. If she picks them to be roughly equal --- \(p \approx q \approx \sqrt{N}\) --- the number is called a \emph{balanced \index{semiprime}semiprime}, and it is the bedrock of RSA encryption. How does the critical depth \(d^*\) relate to \(N\) for these well-balanced locks?
...
\section{A Gallery of Dead Ends}\label{ch7-a-gallery-of-dead-ends}
...
\section{Why Quantum Parallelism Fails at the Fork}\label{ch7-why-quantum-parallelism-fails-at-the-fork}
...
\section{The Sum-to-Zero Principle}\label{ch7-the-sum-to-zero-principle}
...
\section{The Complexity Ladder}\label{ch7-the-complexity-ladder}
...
\emph{In the next chapter, we shall leave the corridors of the Pythagorean tree and step into a different labyrinth entirely --- one built not from right triangles but from the algebra of \index{octonions}octonions, where the rules of multiplication themselves become a source of mathematical wonder.}
