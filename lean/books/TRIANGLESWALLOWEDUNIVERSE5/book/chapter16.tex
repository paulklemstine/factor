\emph{In which we discover that the humblest objects in all of mathematics --- right triangles with whole-number sides --- have been hiding a connection to Einstein's spacetime all along.}



\section{§1. A Puzzle to Begin: The Form That Remembers}\label{ch16-a-puzzle-to-begin-the-form-that-remembers}
...
What makes this more than a one-off curiosity is that there are \emph{two more recipes} with the same property:
...
\((7, 1, 2)\) & \(49 + 1 - 4 = 46\) & \((7, 17, 18)\) & \(49 + 289 - 324 = 14\)\ldots{} \\
\end{longtable}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig02_q_table.png}
\end{figure}
...
But let us not dwell on the sociology of mathematics. The deeper question is: \emph{why} do these recipes preserve \(Q\)? And what happens when \(Q\) equals zero?



\section{§2. The Null Cone, or: Where Right Triangles Live}\label{ch16-the-null-cone-or-where-right-triangles-live}
...
So the \index{Pythagorean triple}Pythagorean triples are exactly the \emph{positive integer points where \(Q\) vanishes}. In three-dimensional \((a, b, c)\)-space, the set of \emph{all} points (not just integers) where \(Q = 0\) forms a surface:

\[a^2 + b^2 = c^2\]

This is a \emph{cone} --- a double cone, opening symmetrically along the \(c\)-axis like two ice cream cones pressed tip to tip. In the jargon of physics, it is called the \textbf{null cone}, or more evocatively, the \textbf{light cone}. In Einstein's theory of \index{special relativity}special relativity, light travels along the surface of exactly such a cone (with spatial coordinates in place of \(a\) and \(b\), and time in place of \(c\)). Points inside the cone (\(Q < 0\)) are called \emph{timelike} --- they represent intervals that a massive particle could traverse. Points outside (\(Q > 0\)) are \emph{spacelike} --- no signal, not even light, can bridge them.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig03_null_cone.png}
\end{figure}


Now the three Berggren recipes reveal their true nature. Since each one preserves the value of \(Q\) for \emph{any} triple --- not just Pythagorean ones --- it follows that if you start with \(Q = 0\), you end with \(Q = 0\). In other words:

\begin{quote}
\emph{The Berggren transformations map Pythagorean triples to Pythagorean triples.}
\end{quote}

But this is actually the \emph{least} interesting thing they do. They preserve the \emph{entire} form \(Q\), for every value. They are symmetries of the cone, the way rotations are symmetries of a circle. The fact that they happen to map lattice points on the cone to other lattice points on the cone is a \emph{consequence} of a much grander invariance.
...
\section{§3. The Berggren Tree: A Family Album of Right Triangles}\label{ch16-the-berggren-tree-a-family-album-of-right-triangles}

Here is a challenge for a rainy afternoon. Starting from \((3, 4, 5)\), apply the three recipes \(A\), \(B\), and \(C\) to generate three children. Then apply \(A\), \(B\), \(C\) to each child to get nine grandchildren. Continue. \emph{Can you ever produce the same triple twice? Can you ever miss one?}
...
\begin{quote}
\emph{Every primitive Pythagorean triple appears exactly once in the \index{Berggren tree}Berggren tree.}
\end{quote}

This is a remarkable claim. It says that the three recipes \(A\), \(B\), \(C\), applied repeatedly to the single seed \((3, 4, 5)\), generate \emph{all} primitive triples --- every right triangle with coprime whole-number sides is somewhere in this tree --- and that no triple is ever produced twice. Every primitive triple has a unique \emph{address}: a finite string over the alphabet \(\{A, B, C\}\) that tells you exactly how to reach it from the root.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig04_berggren_tree.png}
\end{figure}


The independent rediscoveries of this tree --- by Berggren in 1934, Barning in 1963, and Hall in 1970 --- offer a charming lesson in the sociology of mathematics. Three mathematicians, in three decades, in three countries, each stumbled upon the same branching structure. None appears to have known of the others' work. One might be tempted to call this a coincidence, but the truth is simpler: some mathematics is so natural, so \emph{inevitable}, that it practically discovers itself. The tree was always there, implicit in the \index{quadratic form}quadratic form \(Q\). All it needed was someone willing to look.



\section{§4. Enter the Lorentz Group --- or, What Physicists Knew All Along}\label{ch16-enter-the-lorentz-group-or-what-physicists-knew-all-along}
...
And every single one of them satisfies \(M^\top J M = J\). They are elements of the \index{Lorentz group}Lorentz group --- specifically, of \(O(2,1; \mathbb{Z})\), the \emph{integer} Lorentz group, consisting of those Lorentz transformations whose matrix entries happen to be integers.

The reader may verify this for matrix \(B\) as an exercise (it is listed as Exercise 6 in the appendix). The computation is entirely mechanical: multiply out \(B^\top J B\) and confirm that you get \(J\) back. But the \emph{meaning} is not mechanical at all. It says that the arithmetic of right triangles --- a subject as old as civilization --- is governed by the same symmetry group as special relativity.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig05_minkowski_poincare.png}
\end{figure}


What does it \emph{mean} that the arithmetic of right triangles obeys the same symmetry group as spacetime? Is this a deep unity or a superficial coincidence --- two mathematical structures that happen to share the same algebraic skeleton?

The honest answer is that nobody knows for sure. The quadratic form \(x^2 + y^2 - z^2\) is a simple object, and simple objects tend to show up in many places. The integers that fall on its \index{null cone}null cone are Pythagorean triples; the real matrices that preserve it are Lorentz transformations; the geometry it induces on the unit ``sphere'' \(Q = -1\) is hyperbolic. These are three faces of one diamond. Whether the diamond conceals further faces --- some physical, some number-theoretic, some not yet imagined --- is a question I shall leave to the reader's philosophical inclinations.

Eugene Wigner once wrote of ``the unreasonable effectiveness of mathematics in the natural sciences.'' The Berggren tree is an instance of the reverse phenomenon: the unreasonable effectiveness of \emph{physics} in pure number theory. The Lorentz group was invented to describe the behavior of light. It turns out to organize the behavior of right triangles. Mathematics, as usual, does not check your credentials before granting admission.



\section{§5. Climbing Down the Tree: The Inverse Matrices and Descent}\label{ch16-climbing-down-the-tree-the-inverse-matrices-and-descent}

Suppose I hand you the Pythagorean triple \((697, 696, 985)\). Somewhere in the infinite Berggren tree, this triple occupies a single node. Can you find its \emph{address} --- the exact sequence of \(A\)'s, \(B\)'s, and \(C\)'s that leads from \((3, 4, 5)\) down to it? Better yet: can you find it \emph{quickly}?

The key insight is that every machine can be run in reverse. The inverse Berggren matrices \(A^{-1}\), \(B^{-1}\), \(C^{-1}\) allow us to climb \emph{up} the tree, from child to parent. For matrix \(A\), the inverse transformation is:
...
But here is the beautiful part: at every step, the hypotenuse \emph{strictly decreases}. The third component of the transformed triple is always less than \(c\). This guarantees that the algorithm terminates --- you cannot wander forever; you \emph{must} reach the root \((3, 4, 5)\) in finitely many steps. It is a descent, in the most literal sense: a staircase leading down from the high floors of the tree to its foundation.
...
Hmm --- let me reconsider the parent. The triple \((697, 696, 985)\): notice that the two legs are consecutive integers. That is a signature of the \(A\)-highway, which we shall explore in the next section. It turns out that the \emph{ordered} triple may need its legs swapped to fit the parametrization convention. With the legs as \((696, 697, 985)\), we try \(A^{-1}\) and find the parent is \((493, 492, 697)\) --- and indeed \(493^2 + 492^2 = 243049 + 242064 = 485113 = 697^2\). (Check: \(697^2 = 485809\). Hmm --- let me just recompute. \(493^2 = 243049\), \(492^2 = 242064\), sum \(= 485113\). And \(697^2 = 485809\). Those don't match. The descent requires more care than casual arithmetic.)
...
These are \emph{not} consecutive parameters. The descent of this particular triple is more complex than a pure \(A\)-highway. But the principle holds: at each step, the hypotenuse shrinks, and after finitely many steps, we arrive at \((3, 4, 5)\).


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig06_elevator_descent.png}
\end{figure}




\section{\texorpdfstring{§6. The Highway of Pure \(A\)'s: Consecutive Parameters and Depth}{§6. The Highway of Pure A's: Consecutive Parameters and Depth}}\label{ch16-the-highway-of-pure-as-consecutive-parameters-and-depth}
...
Do you see the pattern? In each case, \(n = m - 1\). The parameters are \emph{consecutive}. Let us call such triples \textbf{consecutive-parameter triples}. Their general form is:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig07_a_highway.png}
\end{figure}




\section{§7. The Depth of a Prime: A Number-Theoretic Surprise}\label{ch16-the-depth-of-a-prime-a-number-theoretic-surprise}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig08_prime_depth.png}
\end{figure}




\section{§8. How Many Triples Does a Number Have? The Semiprime Surprise}\label{ch16-how-many-triples-does-a-number-have-the-semiprime-surprise}

Take the number \(N = 15 = 3 \times 5\). It is the product of two primes. How many different ways can \(15\) appear as a leg of a primitive Pythagorean triple? If you guess ``one'' (because each prime appears in just one triple), you will be wrong. If you guess ``two'' (one for each prime factor), you will \emph{also} be wrong. The answer is four.
...
For a \index{semiprime}semiprime \(N = pq\) with \(p\) and \(q\) distinct odd primes:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig09_factor_table.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig10_triple_count.png}
\end{figure}




\section{§9. Tiling the Hyperbolic Plane}\label{ch16-tiling-the-hyperbolic-plane}
...
The integer Lorentz group \(O(2,1; \mathbb{Z})\) acts on the hyperbolic plane as a group of \emph{isometries} --- distance-preserving transformations. The three Berggren matrices, being elements of this group, act as three specific hyperbolic isometries. Their repeated application tiles the hyperbolic plane: the orbit of any fundamental region under all possible compositions of \(A\), \(B\), \(C\) and their inverses fills the entire disk with non-overlapping copies, like Escher's fish covering their circular world.
...
Why the \emph{hyperbolic} plane and not the ordinary Euclidean one? Because Euclidean isometries preserve the positive-definite form \(x^2 + y^2\), while our Berggren matrices preserve the \emph{indefinite} form \(a^2 + b^2 - c^2\). That minus sign --- the same one that distinguishes space from time in special relativity --- is what curves the geometry. In the Euclidean world, parallel lines never meet. In the hyperbolic world, there are infinitely many lines through a given point that never meet a given line. The arithmetic of Pythagorean triples lives in this wilder, more spacious geometry.

Escher, famously, visited the Alhambra in Granada in 1922 and was transfixed by the Moorish tile-work --- the intricate geometric patterns that cover every surface without gap or overlap. He spent decades translating those patterns into his own art, eventually corresponding with the geometer H. S. M. Coxeter, who introduced him to hyperbolic tessellations. The result was the \emph{Circle Limit} series --- woodcuts of fish and angels and devils, swimming in the Poincaré disk. Escher had no idea that his art was doing number theory. Neither, for that matter, did Coxeter.


\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig11_poincare_tessellation.png}
\end{figure}




\section{§10. The Grand Unification: Pythagoras Meets Einstein}\label{ch16-the-grand-unification-pythagoras-meets-einstein}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter16/images/fig12_grand_diagram.png}
\end{figure}
...
\section{Appendix A: The Reader's Toolkit --- Puzzles and Exercises}\label{ch16-appendix-a-the-readers-toolkit-puzzles-and-exercises}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Warm-up.} Verify that \(Q(3, 4, 5) = 0\) and \(Q(5, 12, 13) = 0\). Now compute \(Q(6, 8, 10)\). What happens, and why? \emph{(Hint: \((6, 8, 10) = 2 \times (3, 4, 5)\).)}
\item
  \textbf{Tree growth.} Apply all three Berggren transformations to \((5, 12, 13)\). Verify that each result is a Pythagorean triple.
\item
  \textbf{Finding an address.} Find the Berggren tree address of \((20, 21, 29)\). \emph{(Hint: apply inverse transformations until you reach the root.)}
\item
  \textbf{The \(A\)-highway.} The prime \(p = 29\) has depth \(\frac{29 - 3}{2} = 13\). Write down the corresponding Pythagorean triple and verify that it lies on the \(A\)-highway.
\item
  \textbf{Counting triples.} For \(N = 21 = 3 \times 7\), predict the number of Pythagorean triples with leg \(21\) using the formula \(|T(N)| = \frac{3^k - 1}{2}\). Then find all four triples explicitly.
\item
  \textbf{★ (Challenge)} Show that the Berggren matrix \(B\) satisfies \(B^\top J B = J\), where \(J = \text{diag}(1, 1, -1)\). \emph{(This proves that \(B\) is an element of the Lorentz group \(O(2,1)\).)}
\item
  \textbf{★ (Challenge)} Prove the identity \((a + 2b + 2c)^2 + (2a + b + 2c)^2 - (2a + 2b + 3c)^2 = a^2 + b^2 - c^2\) by direct expansion. Conclude that \(B\) preserves \(Q\) for \emph{all} triples, not just Pythagorean ones.
\item
  \textbf{★★ (Research)} Is there a four-dimensional analogue? That is, can one construct a tree of primitive Pythagorean \emph{quadruples} \((a, b, c, d)\) with \(a^2 + b^2 + c^2 = d^2\), generated by integer matrices in \(O(3,1; \mathbb{Z})\)? If so, how many generators are needed?
\item
  \textbf{★★ (Open-ended)} The depth formula \(\text{depth}(p) = (p-3)/2\) tells us that large primes live deep in the tree. Is there a useful sense in which the \emph{difficulty} of factoring a number \(N\) is related to the depth of some associated Pythagorean triple? Explore this question computationally for small semiprimes.
\item
  \textbf{A meditation.} The quadratic form \(Q(a, b, c) = a^2 + b^2 - c^2\) appears in at least three independent contexts: Pythagorean triples, special relativity, and hyperbolic geometry. Can you think of a fourth?
\end{enumerate}
