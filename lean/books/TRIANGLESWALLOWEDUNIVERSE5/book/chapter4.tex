\section{\texorpdfstring{\emph{How the World's Oldest Equation Secretly Cracks the World's Hardest Codes}}{How the World's Oldest Equation Secretly Cracks the World's Hardest Codes}}\label{ch4-how-the-worlds-oldest-equation-secretly-cracks-the-worlds-hardest-codes}



\section{The Puzzle of the Two Squares}\label{ch4-the-puzzle-of-the-two-squares}
...
\section{Euler's Jewel --- Cracking Numbers with Two Portraits}\label{ch4-eulers-jewel-cracking-numbers-with-two-portraits}
...
The magic of these transformations is that they preserve \(Q\). If \(Q(a, b, c) = 0\), then \(Q(B_i(a, b, c)) = 0\) for \(i = 1, 2, 3\). This is the \textbf{Lorentz Invariance Theorem}, and it is the reason the \index{Berggren tree}Berggren tree works: every child of a Pythagorean triple is itself Pythagorean.
...
\section{The Berggren Tree --- An Infinite Family Album}\label{ch4-the-berggren-tree-an-infinite-family-album}
...
\section{The Tree Sieve --- Shaking the Family Tree for Factors}\label{ch4-the-tree-sieve-shaking-the-family-tree-for-factors}
...
\section{\texorpdfstring{Semiprime Secrets --- Why \(N = p \times q\) Is Special}{Semiprime Secrets --- Why N = p \textbackslash times q Is Special}}\label{ch4-semiprime-secrets-why-n-p-times-q-is-special}
...
\section{The Exponential Climb --- How Fast the Tree Grows}\label{ch4-the-exponential-climb-how-fast-the-tree-grows}
...
\section{The Hidden Arithmetic of Products}\label{ch4-the-hidden-arithmetic-of-products}
...
\section{The Three Roads Converge --- A Factoring Toolkit}\label{ch4-the-three-roads-converge-a-factoring-toolkit}
...
And yet the ideas in this chapter are not idle curiosities. They are the \emph{ancestors} of the methods that \emph{do} work in practice. The \index{quadratic sieve}quadratic sieve and the \index{number field sieve}number field sieve --- the fastest known classical factoring algorithms --- are direct descendants of the idea that smooth relations, combined algebraically, can reveal factors. And Shor's quantum algorithm, which would factor large semiprimes in polynomial time on a quantum computer, exploits the same multiplicative structure that Brahmagupta discovered fourteen centuries ago.

In the chapters to come, we will sharpen these tools. We will learn about \index{lattice reduction}lattice reduction, which searches the Berggren tree more efficiently by exploiting the geometry of high-dimensional lattices. We will encounter complexity bounds --- precise statements about how many steps the sieve requires --- and discover that the answer scales as \(\Theta(\sqrt{N})\), tantalizingly close to but not quite polynomial. And we will peer into the quantum future, where the interference patterns of qubits accomplish in \(O(N^{1/4})\) steps what classical computers struggle to do at all.
...
\section{Puzzles for the Reader}\label{ch4-puzzles-for-the-reader}
