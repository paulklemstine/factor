...
Q(a, b, c) = a^2 + b^2 - c^2, establishing them as elements of the
...
/-- Berggren matrix A preserves the Lorentz form.
    If (a,b,c) satisfies a^2 + b^2 = c^2, then so does
...
/-- The Lorentz form Q(a,b,c) = a^2+b^2-c^2 is preserved by all three
...
/-- For the consecutive-parameter case (n = m-1), the Berggren depth
    ...
    (2m-1, 2m(m-1), 2m^2-2m+1) has depth m-2 in the Berggren tree,
...
/-- The counting theorem for semiprimes: n = p x q has exactly 4 Pythagorean triples.
    This is because sigma_0(n^2) = sigma_0(p^2q^2) = 3 x 3 = 9, so |T(n)| = (9-1)/2 = 4. -/
theorem semiprime_four_triples (p q : Nat) (_hp : Nat.Prime p) (_hq : Nat.Prime q)
    (_hpq : p != q) (_hodd_p : p % 2 = 1) (_hodd_q : q % 2 = 1) :
    -- The number of divisors of (p*q)^2 that are less than p*q
    -- equals 4 (for distinct odd primes p != q)
    -- The divisor pairs are: (1, p^2q^2), (p, pq^2), (q, qp^2), (p^2, q^2)
    let n := p * q
    1 * (n ^ 2) = n ^ 2 /\
    p * (p * q ^ 2) = n ^ 2 /\
    q * (q * p ^ 2) = n ^ 2 /\
