...
Fermat claimed in 1637 to have "a truly marvelous proof" of the statement
  ...
  forall n >= 3, there are no positive integers a, b, c with a^n + b^n = c^n
...
argument in Int[zeta_n] (the cyclotomic integers) that assumes unique factorization.
...
/-- Fermat's Last Theorem: no positive integer solutions to a^n + b^n = c^n for n >= 3. -/
...
PROBLEM
===============================================================================
S2: REDUCTION TO PRIME EXPONENTS
===============================================================================
...
-- ===============================================================================
--  S3: THE CASE n = 4 --- Fermat's Infinite Descent (fits in a margin!)
-- ===============================================================================
...
Actually, Fermat proved the stronger result: a^4 + b^4 = c^2 has no
positive integer solutions. This immediately implies FLT for n = 4
since a^4 + b^4 = c^4 would give a^4 + b^4 = (c^2)^2.

**Proof sketch (infinite descent):**
Suppose a^4 + b^4 = c^2 with a, b, c > 0 and c minimal.
Then (a^2, b^2, c) is a Pythagorean-like triple. Factor and descend to
...
/-
PROBLEM
FLT for n = 4: a^4 + b^4 != c^4 for positive integers.
...
/-
PROBLEM
The stronger form: a^4 + b^4 != c^2 for positive integers.
...
PROBLEM
===============================================================================
S4: THE CASE n = 3 --- Euler's Proof
===============================================================================

FLT for n = 3: a^3 + b^3 != c^3 for positive integers.
...
-- ===============================================================================
--  S6: WHY NO MARGIN PROOF EXISTS (Informal Argument)
-- ===============================================================================
...
Fermat likely attempted to prove FLT by factoring in Int[zeta_n], the ring
of integers of the cyclotomic field Rat(zeta_n), where zeta_n = e^{2pii/n}.

The argument would go:
1. From a^n + b^n = c^n, factor the left side:
   (a + b)(a + zetab)(a + zeta^2b)...(a + zeta^n^-^1b) = c^n
2. If these factors are pairwise coprime and Int[zeta_n] has unique
   factorization, then each factor must be an n-th power.
3. Derive a contradiction.

**The flaw:** Int[zeta_n] does NOT always have unique factorization!
It fails for n = 23 (discovered by Kummer, 1847).

Kummer saved the approach for "regular primes" (primes p where p
does not divide the class number of Rat(zeta_p)), proving FLT for all
...
- n = 4 (Fermat's infinite descent) check
- n = 3 (Euler, with a small gap) check
- n = 5 (Dirichlet/Legendre, 1825) --- short but not trivial
- n = 7 (Lame, 1839) --- getting long
