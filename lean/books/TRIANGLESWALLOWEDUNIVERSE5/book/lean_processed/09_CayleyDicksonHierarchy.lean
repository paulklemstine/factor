...
- Quaternion -> Octonion: Lose associativity (gain: E_8 lattice)
- Octonion -> Sedenions: Lose division (CHANNEL BREAKS --- zero divisors appear)
-/
...
PROVIDED SOLUTION
Given a^2 = n, use (a, 0) since a^2 + 0^2 = n.
-/
theorem channel_1_to_2 (n : Nat) (h : exists a : Int, a ^ 2 = upn) :
    exists a b : Int, a ^ 2 + b ^ 2 = upn := by
...
PROVIDED SOLUTION
Given a^2 + b^2 = n, use (a, b, 0, 0) since a^2 + b^2 + 0^2 + 0^2 = n.
-/
theorem channel_2_to_3 (n : Nat) (h : exists a b : Int, a ^ 2 + b ^ 2 = upn) :
    exists a b c d : Int, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = upn := by
...
PROVIDED SOLUTION
Given a^2 + b^2 + c^2 + d^2 = n, define f : Fin 8 -> Int by f(0)=a, f(1)=b, f(2)=c, f(3)=d, f(4..7)=0. Then Sum f(i)^2 = a^2 + b^2 + c^2 + d^2 + 0 + 0 + 0 + 0 = n. Use Fin.sum_univ_eight or Fin.cons.
-/
theorem channel_3_to_4 (n : Nat) (h : exists a b c d : Int, a ^ 2 + b ^ 2 + c ^ 2 + d ^ 2 = upn) :
    exists f : Fin 8 -> Int, Sum i, f i ^ 2 = upn := by
...
/-- Sum of all Hurwitz dimensions equals 15, which is 2^4 - 1 -/
theorem sum_hurwitz_dims : 1 + 2 + 4 + 8 = 15 := by norm_num

/-- Product of all Hurwitz dimensions equals 64 = 2^6 -/
...
PROVIDED SOLUTION
The solutions to a^2 = n in [-n, n] are at most {sqrtn, -sqrtn}, so the cardinality is at most 2. Show the filter is a subset of {Nat.sqrt n, -(Nat.sqrt n)} and use Finset.card_le_card.
-/
theorem channel_1_bounded (n : Nat) (hn : n >= 1) :
    (Finset.filter (fun a : Int => a ^ 2 = upn) (Finset.Icc (-(upn : Int)) upn)).card <= 2 := by
  refine' le_trans ( Finset.card_le_card _ ) _;
  exact { up ( Nat.sqrt n ), -up ( Nat.sqrt n ) };
