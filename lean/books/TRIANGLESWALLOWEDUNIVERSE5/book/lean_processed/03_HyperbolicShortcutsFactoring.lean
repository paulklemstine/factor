...
A "hyperbolic shortcut" is a composite matrix B_{i_1} . B_{i_2} . ... . B_{i_k}
...
/-! ## S1. Fundamental Algebraic Identities -/
...
/-! ## S5. Hyperbolic Shortcut Composition -/
...
theorem shortcut_compose (p q : BPath) :
    tripleAt (p ++ q) = pathMat p *_v tripleAt q := by
...
/-- The hypotenuse strictly increases under B_1 when a^2 + b^2 = c^2, a > 0, c > 0. -/
theorem B_1_hyp_increases (a b c : Int) (ha : 0 < a) (hc : 0 < c)
    (h : a ^ 2 + b ^ 2 = c ^ 2) :
    c < 2*a - 2*b + 3*c := by nlinarith [sq_nonneg (c - b)]

/-- The hypotenuse strictly increases under B_3 when a^2 + b^2 = c^2, b > 0, c > 0. -/
...
/-! ## S7. Middle Branch Chebyshev Recurrence -/

/-- B_2 applied once to (3,4,5) gives hypotenuse 29. -/
theorem B_2_iter1_hyp :
    (B_2 *_v (![3, 4, 5] : Fin 3 -> Int)) 2 = 29 := by native_decide

/-- B_2 applied twice gives hypotenuse 169. -/
theorem B_2_iter2_hyp :
    (B_2 *_v (B_2 *_v (![3, 4, 5] : Fin 3 -> Int))) 2 = 169 := by native_decide

/-- B_2 applied three times gives hypotenuse 985. -/
theorem B_2_iter3_hyp :
    (B_2 *_v (B_2 *_v (B_2 *_v (![3, 4, 5] : Fin 3 -> Int)))) 2 = 985 := by native_decide
...
/-! ## S8. Inverse Berggren Matrices (Tree Ascent) -/
...
/-! ## S12. Lorentz Form as Quadratic Invariant -/
