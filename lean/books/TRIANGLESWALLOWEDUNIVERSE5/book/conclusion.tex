\section{\texorpdfstring{Conclusion --- \emph{The Rosetta Stone}}{Conclusion --- The Rosetta Stone}}\label{concl-conclusion-the-rosetta-stone}



\section{The Rope-Stretcher's Return}\label{concl-the-rope-stretchers-return}
...
\textbf{Face 2: A Point on Einstein's Light Cone.} The \index{quadratic form}quadratic form \(Q(a, b, c) = a^2 + b^2 - c^2\) is the Minkowski metric of \((2{+}1)\)-dimensional spacetime. Our \index{Pythagorean triple}Pythagorean triples live on the \index{null cone}null cone \(Q = 0\) --- the same surface that light rays trace through the cosmos. The Berggren matrices generate a discrete subgroup of the \index{Lorentz group}Lorentz group \(O(2,1;\mathbb{Z})\), and the formula for inverting them,

\[
\mathbf{B}^{-1} = \mathbf{Q} \cdot \mathbf{B}^{\!\top} \cdot \mathbf{Q},
\]

is precisely the formula relativists use to invert Lorentz boosts. Minkowski's geometry, born to describe continuous spacetime, secretly organizes the most ancient discrete objects in number theory. The irony is exquisite: the pharaoh's surveyor was doing \index{special relativity}special relativity with a rope.
...
is the norm multiplicativity of the \index{Gaussian integers}Gaussian integers \(\mathbb{Z}[i]\). It extends to Euler's four-square identity via the \index{quaternions}quaternions \(\mathbb{H}\), and to Degen's eight-square identity via the \index{octonions}octonions \(\mathbb{O}\). Each step up the ladder doubles the dimension and loses a structural property --- commutativity, then associativity, then the division property --- but each step also \emph{doubles the number of factoring channels}. A Pythagorean octuplet yields twenty-eight independent chances to find \(\gcd(\cdot, N) \notin \{1, N\}\). Hurwitz's theorem tells us this ladder has exactly four rungs --- \(1, 2, 4, 8\) --- and no more. The channel hierarchy is not a human choice; it is a law of algebra.
...
\textbf{Face 9: The Only Exponent That Works.} \index{Fermat's Last Theorem}Fermat's Last Theorem tells us that \(a^n + b^n = c^n\) has no positive integer solutions for \(n \geq 3\). The Pythagorean world --- \(n = 2\) --- is the \emph{unique} exponent where integer solutions are not merely possible but infinitely abundant, richly structured, and computationally useful. Fermat himself proved the \(n = 4\) case using \index{infinite descent}infinite descent --- the \emph{same} descent principle that drives Berggren tree factoring. The Pythagorean equation sits at the exact boundary where descent works. Beyond it lies Fermat's empty sea, and the full proof required Wiles, Taylor, and the deepest machinery of twentieth-century mathematics.
...
\section{The Unreasonable Effectiveness of a Schoolchild's Equation}\label{concl-the-unreasonable-effectiveness-of-a-schoolchilds-equation}
...
\section{The Puzzles That Remain}\label{concl-the-puzzles-that-remain}
...
\section{A Last Puzzle}\label{concl-a-last-puzzle}
