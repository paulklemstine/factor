\chapter{\texorpdfstring{\emph{The Tree That Cracks Numbers}}{The Tree That Cracks Numbers}}
\section{\texorpdfstring{\emph{How a Babylonian Equation Grows a Forest That Can Split Integers Apart}}{How a Babylonian Equation Grows a Forest That Can Split Integers Apart}}
...
\caption{}
\end{figure}
A small inset panel beside the tree. A single triple at a leaf node has an arrow pointing upward, labeled ``\(B^{-1}\)'', tracing the unique path back to the root \((3,4,5)\). Caption: ``Climbing the tree: from any triple, there is exactly one path back to the root.''{]}
...
Suppose I hand you the triple \((39, 80, 89)\) and ask: which of the three branches produced it? You must figure out its \emph{parent} in the Berggren tree. There is a beautiful fact: you don't need to guess. The arithmetic tells you.
...
There is a deep reason for all of this, and it involves a geometry that would have delighted Einstein. The Pythagorean equation \(a^2 + b^2 = c^2\) can be rewritten as \(a^2 + b^2 - c^2 = 0\) --- the equation of a \emph{light cone} in \(2+1\)-dimensional spacetime. The Berggren transformations, and their inverses, are integer \textbf{Lorentz transformations}: they belong to the group \(O(2,1;\mathbb{Z})\), the same symmetry group that governs special relativity, restricted to integer entries. We'll return to this cosmic connection in Section 8. For now, the point is practical: these maps are geometrically natural and algebraically exact.
...
\caption{}
\end{figure}
A bar chart comparing ``number of descent steps'' (vertical axis) versus \(N\) (horizontal axis, showing \(15, 21, 77, 143, 323\)). The bars increase in height. A faint dashed curve proportional to \(\sqrt{N}\) is overlaid in gray, hinting at the asymptotic growth. A caption reads: ``The climb gets longer --- but always ends.''{]}
...
\item
  \emph{Is there a connection to lattice reduction?} The Berggren tree, viewed as a set of integer points on the light cone, is intimately related to shortest vectors in certain lattices. The Lattice-Tree Correspondence is developed in Chapter 2.
\end{itemize}
