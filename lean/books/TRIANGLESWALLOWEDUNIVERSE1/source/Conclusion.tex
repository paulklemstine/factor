...
\textbf{Face 3: The Euclidean Algorithm in Disguise.} Climbing \emph{down} the Berggren tree --- from child to parent --- turns out to be the Euclidean algorithm applied to the parameter ratio \(m/n\) of the classical Pythagorean parametrization. The \(2 \times 2\) inverse matrices compute continued-fraction quotients; the descent terminates at the root \((3, 4, 5)\) in exactly as many steps as \(\gcd(m, n)\) takes to reach \(1\). The most ancient algorithm in mathematics --- Euclid's recipe for finding greatest common divisors, circa 300 BCE --- was hiding inside a tree discovered in 1934.

\textbf{Face 4: A Lattice Vector.} Each Pythagorean triple is a point in the integer lattice \(\mathbb{Z}^3\), and its Berggren ancestors trace a path through lattice basis reduction. In two dimensions, Gauss showed this reduction is already optimal: no shortcut can beat the zigzag descent, and the complexity is \(\Theta(\sqrt{N})\) for a balanced semiprime \(N\). This is not a failure of the method --- it is a \emph{theorem} about the geometry of planar lattices. But in three and more dimensions, the walls of optimality crack open. The LLL algorithm exploits extra dimensions to find short vectors that the planar projection misses entirely, and this is where the higher-dimensional Pythagorean objects --- quadruples, quintuplets, octuplets --- begin to earn their keep.
...
Now descend the tree. At each node, compute \(\gcd(\text{leg}, N)\). If a nontrivial divisor appears --- and it \emph{must}, since the hypotenuse strictly decreases and cannot reach zero without passing through a node where the leg shares a factor with \(N\) --- the composite is cracked. The difference-of-squares identity
...
\textbf{Face 7: A Quantum Search Problem.} The tree descent is deterministic --- at each node, at most one inverse branch produces a valid child --- so quantum parallelism cannot explore multiple branches simultaneously. But Grover's algorithm \emph{can} compress the sequential depth search from \(O(d^*)\) to \(O(\sqrt{d^*})\), yielding

\[
O(\sqrt{d^*}) = O(N^{1/4})
\]

for balanced semiprimes. This is a genuine quadratic speedup, and it is \emph{tight}: Bennett, Bernstein, Brassard, and Vazirani proved in 1997 that Grover's bound cannot be improved. The Berggren tree reveals exactly \emph{where} quantum mechanics helps (sequential search) and \emph{where} it cannot (deterministic branching).
...
\textbf{Face 9: The Only Exponent That Works.} Fermat's Last Theorem tells us that \(a^n + b^n = c^n\) has no positive integer solutions for \(n \geq 3\). The Pythagorean world --- \(n = 2\) --- is the \emph{unique} exponent where integer solutions are not merely possible but infinitely abundant, richly structured, and computationally useful. Fermat himself proved the \(n = 4\) case using infinite descent --- the \emph{same} descent principle that drives Berggren tree factoring. The Pythagorean equation sits at the exact boundary where descent works. Beyond it lies Fermat's empty sea, and the full proof required Wiles, Taylor, and the deepest machinery of twentieth-century mathematics.
...
What we \emph{do} know is the complexity landscape, and it is worth stating plainly:
...
The Pythagorean framework sits at the \(\sqrt{N}\) boundary --- too slow for breaking modern cryptographic keys, but rich enough to illuminate the \emph{structure} of factoring in ways that sub-exponential methods, which rely on smoothness heuristics rather than geometric insight, do not. The question is whether the higher-dimensional extensions --- the quadruple forests, the octuplet channels, the lattice lifts --- can cross the barrier into sub-exponential territory. The answer is unknown. The question is magnificent.
...
\caption{}
\end{figure}
The final image. A single \(3\)-\(4\)-\(5\) right triangle, drawn in clean black ink on an otherwise empty white page. At the right-angle vertex, a tiny door stands ajar, and through it spills warm golden light. The light illuminates, faintly, the outlines of vast mathematical structures receding into the distance --- trees, lattices, cones, towers, circuits --- all contained, impossibly, within the interior of the triangle. Beneath the triangle, in small italic type: \emph{``The simplest objects in mathematics hide the deepest structures.''}{]}
