...
### *In which a party trick reveals a deep identity, an infinite tree hides the symmetries of spacetime, and a greatest common divisor cracks a number wide open*
...
"I am thinking of a number whose square is $441$. Can you factor $441$ into two smaller numbers — *neither of them* $1$ or $441$ — in under five seconds?"
...
[ILLUSTRATION: A right triangle with legs labeled $21$ and $20$ and hypotenuse labeled $29$. On the leg of length $21$, a square of area $441$ is drawn. This square is visually "cracked" along a horizontal line into two rectangular pieces: a thin rectangle of dimensions $9 \times 49$ sitting below, and the crack line annotated with the equation $(29-20)\times(29+20) = 9 \times 49 = 441$. The numbers $3^2$ and $7^2$ appear faintly inside the two rectangles.]
...
But notice what the identity *gives* you. If $a$ is composite — say $a = pq$ — then $(c-b)(c+b) = (pq)^2$, and the factors $c - b$ and $c + b$ may reveal the decomposition. In our example, $c - b = 29 - 20 = 9$ and $c + b = 29 + 20 = 49$, and these are exactly $3^2$ and $7^2$. The triangle has split the number into its prime constituents, like a prism splitting white light into colors.
...
[ILLUSTRATION: A table with six rows and six columns. The columns are labeled $a$, $b$, $c$, $c - b$, $c + b$, and "Revealed factors of $a$." The rows contain the Pythagorean triples $(3,4,5)$, $(5,12,13)$, $(8,15,17)$, $(7,24,25)$, $(21,20,29)$, and $(9,40,41)$. The last column shows, for example, $\gcd(9, 21) = 3$ and $\gcd(1, 5) = 1$. Triples where the GCD reveals a nontrivial factor are highlighted in gold.]

The idea of factoring by difference of squares is ancient. Diophantus of Alexandria, writing around 250 AD, was fascinated by representing numbers as differences of squares — his *Arithmetica* is filled with problems of this type. Fourteen centuries later, Pierre de Fermat transformed the same idea into a systematic factoring method: to factor $N$, search for an integer $x$ such that $x^2 - N$ is a perfect square $y^2$, giving $N = (x - y)(x + y)$. The Pythagorean version of this trick — using a right triangle to supply the difference of squares for free — is, in some sense, a structured variant of Fermat's method, with the Berggren tree (which we are about to meet) serving as an organized catalogue of candidate splittings.

But we are getting ahead of ourselves. Before we can use the tree, we must plant it.

---

## A Tree That Grows All Right Triangles
...
Each of these children, in turn, has three children of its own. And each grandchild has three children. The tree branches outward forever, and — here is the astonishing claim — *every primitive Pythagorean triple appears exactly once*. No duplicates, no omissions. The tree is a perfect filing cabinet for the infinite.

But what is the *rule* that produces the children from the parent?
...
[ILLUSTRATION: An elegant ternary tree, drawn to resemble an actual tree with organic branching. The root node is a circle containing $(3,4,5)$. Three branches descend, labeled $B_1$ (left), $B_2$ (middle), $B_3$ (right), leading to nodes $(5,12,13)$, $(21,20,29)$, $(15,8,17)$. Each of these has three children of its own, forming nine grandchild nodes. The full set of grandchildren: from $(5,12,13)$: $(7,24,25)$, $(55,48,73)$, $(45,28,53)$; from $(21,20,29)$: $(39,80,89)$, $(119,120,169)$, $(77,36,85)$; from $(15,8,17)$: $(33,56,65)$, $(65,72,97)$, $(35,12,37)$. The tree should feel lush and fractal, suggesting infinite continuation.]

But *why* do the Berggren matrices preserve the Pythagorean equation? Let us check one case explicitly. Suppose $(a, b, c)$ satisfies $a^2 + b^2 = c^2$, and consider the child produced by $B_2$:
...
The algebra, when done without error, shows that the quantity $a^2 + b^2 - c^2$ is *exactly preserved* by each Berggren matrix — it is neither created nor destroyed, merely inherited. Since the root satisfies $3^2 + 4^2 - 5^2 = 0$, every descendant satisfies the same equation. The Pythagorean property propagates down the tree like a genetic trait that never mutates.

[ILLUSTRATION: A "zoom-in" panel showing the explicit matrix-vector multiplication $B_2 \times (3, 4, 5)^\top = (21, 20, 29)^\top$. The multiplication is laid out step by step in a decorative frame, with each row of the matrix dotted with the input vector and the partial sums shown. Below, the verification $21^2 + 20^2 = 29^2$ is displayed in a satisfying box.]

This is striking, but the *reason* for the preservation is far more striking still. It has to do with the geometry of spacetime.
...
The matrix $\mathbf{Q}$ is the *Lorentz metric* — the mathematical object that encodes the geometry of Minkowski space. Its signature $(+, +, -)$ is the hallmark of spacetime as opposed to ordinary Euclidean space, where all signs would be positive. And the Pythagorean equation $a^2 + b^2 = c^2$ is simply the statement that the vector $(a, b, c)$ has *zero Lorentzian length* — it lies on the null cone.

Now here is the stunning fact. Each Berggren matrix $B_i$ satisfies the equation:

$$B_i^\top \, \mathbf{Q} \, B_i = \mathbf{Q}.$$

Read that aloud: "$B_i$ transpose, times the Lorentz metric, times $B_i$, equals the Lorentz metric." This is the *defining equation* for a Lorentz transformation. In physics, a Lorentz transformation is a change of reference frame that preserves the speed of light — it is the mathematical expression of Einstein's principle that all inertial observers agree on whether an event is on the light cone. The Berggren matrices are *integer Lorentz transformations*. They belong to the group $O(2, 1; \mathbb{Z})$: the set of all integer-valued matrices that preserve the Lorentz quadratic form.
...
[ILLUSTRATION: A three-dimensional coordinate system with axes labeled $a$, $b$, $c$. A double cone (the "light cone") defined by $a^2 + b^2 = c^2$ opens upward and downward from the origin. The upper cone has several lattice points plotted on its surface, each marked with its coordinates: $(3,4,5)$ near the apex, $(5,12,13)$ and $(21,20,29)$ and $(15,8,17)$ further out. Dashed lines connect parent to child, retracing the edges of the Berggren tree. The cone's surface is semi-transparent, and a faint grid of integer lattice points fills the background, emphasizing that only a select few land exactly on the cone.]
...
One more detail before we move on: the determinants. Compute $\det B_1 = 1$, $\det B_2 = -1$, $\det B_3 = 1$. In the language of physics, $B_1$ and $B_3$ are *proper* Lorentz transformations (they preserve orientation), while $B_2$ is *improper* (it reverses orientation — a reflection combined with a boost). The distinction is physically meaningful: proper Lorentz transformations can be continuously connected to the identity; improper ones cannot. But for the tree's purposes, the distinction is merely cosmetic — all three branches produce valid Pythagorean triples regardless.

[ILLUSTRATION: A whimsical cartoon of a photon (depicted as a small glowing orb with googly eyes and a mortarboard cap) standing at a blackboard. On the blackboard is written the equation $B_i^\top \mathbf{Q}\, B_i = \mathbf{Q}$, and underneath it, in the photon's handwriting: "This is why I always travel at the same speed." Caption: "Even light obeys the Berggren matrices."]

---

## The Invariant That Refuses to Change

Suppose I give you a peculiar machine with three buttons, labeled **L**, **M**, and **R**. You start with any triple of integers $(a, b, c)$ — it need not be Pythagorean. Every time you press a button, the machine transforms your triple according to the corresponding Berggren matrix. Press **M**, and $(a, b, c)$ becomes $(a + 2b + 2c,\; 2a + b + 2c,\; 2a + 2b + 3c)$. Press **L**, and it becomes $(a - 2b + 2c,\; 2a - b + 2c,\; 2a - 2b + 3c)$. And so on.
...
The $B_i^\top \mathbf{Q} \, B_i = \mathbf{Q}$ identity does all the work. This holds for *every* vector $\mathbf{v} \in \mathbb{Z}^3$, not just for null vectors. The quadratic form $Q$ is a *conserved quantity* — a number that the Berggren matrices are constitutionally incapable of altering, no matter how many times they act.
...
[ILLUSTRATION: A flow diagram. A vector $(a, b, c)$ enters a "black box" labeled $B_2$ from the left. Above the input, a circular gauge with a needle pointing to $0$ is labeled "$Q$-meter." The vector emerges on the right as $(a + 2b + 2c,\; 2a + b + 2c,\; 2a + 2b + 3c)$. Above the output, an identical gauge also reads $0$. A banner across the top reads: "The Lorentz form is a conserved quantity." Below the diagram, a smaller example shows a non-null input $(1, 1, 0)$ with $Q = 2$ entering the same box and emerging with $Q$ still reading $2$.]
...
The address of $(3, 4, 5)$ is the empty word — it is the root, and you need not go anywhere. The address of $(5, 12, 13)$ is simply **L** (take the left branch). The address of $(21, 20, 29)$ is **M** (take the middle). The address of $(15, 8, 17)$ is **R**. For deeper nodes, the addresses grow longer: $(119, 120, 169)$ lives at **MM** (two middle steps), and $(39, 80, 89)$ lives at **LM** (left, then middle).
...
Hmm — that gives $(55, 48, 73)$, not $(39, 80, 89)$. Let me reconsider. The address **LM** means: first apply $B_L = B_1$ to the root to reach the first-level node, then apply $B_M = B_2$ to *that* node. So the triple at address $d_1 d_2 \cdots d_k$ is:

$$\mathbf{v}_{d_1 d_2 \cdots d_k} = B_{d_k} \cdots B_{d_2} \cdot B_{d_1} \cdot \begin{pmatrix} 3 \\ 4 \\ 5 \end{pmatrix}.$$

Wait — the order matters. In a tree, the path from the root is read left to right: first you take step $d_1$, arriving at the first-level node $B_{d_1} \cdot \mathbf{v}_0$. Then you take step $d_2$ from *that* node, arriving at $B_{d_2} \cdot (B_{d_1} \cdot \mathbf{v}_0) = B_{d_2} B_{d_1} \mathbf{v}_0$. The matrix product is read *right to left*, as is standard for composition. So address **LM** gives:
...
And the triple $(39, 80, 89)$ must live at a different address. Let us find it by checking: does $(39, 80, 89)$ appear among the grandchildren of the root? Consulting the tree from §2, $(39, 80, 89)$ is the left child of $(21, 20, 29)$ — so its address is **ML**.
...
Now define the *path matrix* for an address $p = d_1 d_2 \cdots d_k$ as the product:
...
Here is the key observation — the **shortcut theorem**. If you concatenate two addresses $p$ and $q$ (first follow path $p$ from the root, then follow path $q$ from wherever you land), the path matrix for the combined address $p \cdot q$ is:
...
[ILLUSTRATION: A map-style diagram of the Berggren tree's first three levels, drawn as a road network or subway map. Each "station" (node) displays the triple on a signpost. Each "line" (edge) is labeled L, M, or R. The path from root $(3,4,5)$ through $(21,20,29)$ to $(39,80,89)$ is highlighted in red, with the label **ML** appearing as a route number. A compass rose in the corner gives the convention: L = left branch, M = middle branch, R = right branch.]
...
The trick is *repeated squaring* — one of the oldest ideas in computational mathematics, dating back to the method of binary expansion used in ancient Egyptian multiplication. The path matrix for $k$ consecutive middle steps is $B_2^k$. And $B_2^k$ can be computed by squaring:
...
This is what I call the *hyperbolic shortcut*: exponentiating a Lorentz transformation to leap across astronomical depths of the tree in logarithmic time. The name is not fanciful — the Berggren matrices are elements of the Lorentz group, which is also the isometry group of the hyperbolic plane, and repeated squaring corresponds to following a geodesic (straightest possible path) through hyperbolic space. The shortcut is literally a shortcut through curved geometry.
...
[ILLUSTRATION: A visual "zoom" effect. On the left, the top of the Berggren tree is shown in full detail (three levels, all nodes visible). On the right, a telescope or "wormhole" graphic shows a single dramatic curved arrow labeled "$B_2^{1{,}000{,}000}$" leaping from the root directly to a node astronomically far down the middle branch, bypassing all intermediate nodes. The intermediate nodes are shown as a blur of receding dots, like stars during a hyperspace jump in a science fiction film. A small counter in the corner reads "20 matrix multiplications."]

The idea of repeated squaring has been reinvented so many times across human history that it resists attribution to a single discoverer. The Rhind Mathematical Papyrus (c. 1550 BCE) describes a method of multiplication by doubling and adding — the same binary decomposition idea, applied to integers rather than matrices. The Indian mathematician Piṅgala described binary representations in his *Chandaḥśāstra* around the 2nd century BCE. In the modern era, the technique is the backbone of public-key cryptography: the RSA algorithm relies on computing $m^e \bmod n$ for enormous exponents $e$, and repeated squaring makes this feasible. The same trick that lets us navigate the Berggren tree in logarithmic time also lets your browser verify a digital signature in milliseconds.

---

## The Elevator Going Up: Inverse Matrices and Tree Ascent
...
The compass is the *inverse Berggren matrix*. Each of the three matrices $B_i$ has an inverse $B_i^{-1}$ — a matrix that undoes the transformation. We already know from the previous chapter that the Berggren matrices have determinant $\pm 1$, which guarantees that their inverses have integer entries. (This is the superpower of the special linear group and its Lorentzian cousin: invertibility over the integers, with no fractions required.) The explicit inverses are:
...
But here is the beautiful part — and the part that connects back to the physics. The inverses arise from the *Lorentz adjoint*:
...
Now for the **ascent algorithm**. Given an unknown primitive triple $\mathbf{v}$, try applying each of the three inverse matrices $B_1^{-1}$, $B_2^{-1}$, $B_3^{-1}$. Exactly one will produce a triple with all-positive entries and a *smaller* hypotenuse. That is your parent. Repeat until you reach $(3, 4, 5)$, and read off the address in reverse.
...
All positive! And $29 < 89$. This is the parent. So the last step was a $B_1$ (left branch), meaning the last letter of the address is **L**.
...
The root! And the step was a $B_2$ (middle branch), so the second-to-last letter is **M**. Reading the address from root to node: **ML**. Exactly as we found in §5.

[ILLUSTRATION: The Berggren tree from §2, but now with upward-pointing arrows drawn in a contrasting color (gold against the tree's green). The arrows are labeled $B_1^{-1}$, $B_2^{-1}$, $B_3^{-1}$. A highlighted path in bright red shows the "elevator ride" from $(39, 80, 89)$ up through $(21, 20, 29)$ to the root $(3, 4, 5)$. At each step, the inverse matrix used and the resulting triple are displayed in a small annotation box.]

[ILLUSTRATION: A side-by-side display of $B_2$ and $B_2^{-1}$. Between them, the Lorentz adjoint formula $B_2^{-1} = \mathbf{Q}\, B_2^\top\, \mathbf{Q}$ is written in an elegant calligraphic font. Arrows show the three-step process: first multiply by $\mathbf{Q}$ (which flips the sign of the third row/column), then transpose, then multiply by $\mathbf{Q}$ again. The visual metaphor is a mirror reflection — the inverse is the original matrix seen "from the other side of the looking glass."]

---

## Why the Hypotenuse Always Grows (and How Fast)

Here is a strange claim: once you leave the root of the Berggren tree, you can never return to a smaller hypotenuse by going *deeper*. The tree only grows. But does every branch grow at the same rate, or does one sprint ahead while another ambles?

To answer this, recall the formulas for the hypotenuse of each child. If the parent is $(a, b, c)$, then:

- **Left branch ($B_1$):** new hypotenuse $= 2a - 2b + 3c$.
- **Middle branch ($B_2$):** new hypotenuse $= 2a + 2b + 3c$.
- **Right branch ($B_3$):** new hypotenuse $= -2a + 2b + 3c$.

Let us check that each of these exceeds $c$, the parent's hypotenuse.

The **middle branch** is obvious: $2a + 2b + 3c > 3c > c$ whenever $a, b > 0$. The middle child always has a strictly larger hypotenuse than its parent, by a margin of at least $2a + 2b$.

The **left branch** requires a moment's thought. We need $2a - 2b + 3c > c$, or equivalently $2a - 2b + 2c > 0$, i.e., $a - b + c > 0$. Since $c > 0$ and $c > b$ (from the Pythagorean equation), we have $c - b > 0$, so $a + (c - b) > 0$. Done.

The **right branch** is similar: we need $-2a + 2b + 3c > c$, or $-a + b + c > 0$. Since $c > a$ (again from the Pythagorean equation), we have $c - a > 0$, so $b + (c - a) > 0$. Done.

So the hypotenuse *always* increases — but the middle branch increases fastest. In fact, the middle branch is explosive. Starting from $(3, 4, 5)$ with $c = 5$:
...
[ILLUSTRATION: A bar chart with three groups of bars, one for each branch (Left, Middle, Right), showing the hypotenuses at levels 0 through 4. The Middle branch bars rocket upward exponentially (5, 29, 169, 985, 5741), dwarfing the other two branches. The Left branch grows modestly (5, 13, 25, 41, 61...) and the Right branch similarly (5, 17, 37, 65...). The visual contrast — one skyscraper among bungalows — makes the exponential-vs.-moderate growth vivid.]
...
The pattern is a *linear recurrence*:
...
The name "Chebyshev" enters because recurrences of the form $c_{n+1} = \alpha \, c_n - c_{n-1}$ are intimately connected to **Chebyshev polynomials of the first kind**. These are the polynomials $T_n(\cos\theta) = \cos(n\theta)$ — a family of functions that arise whenever one studies powers of $2 \times 2$ matrices, trigonometric identities, or best approximation in function spaces. The connection is not superficial: the eigenvalues of $B_2$, restricted to the relevant two-dimensional invariant subspace, generate a recurrence that is a Chebyshev polynomial in disguise.
...
[ILLUSTRATION: A number line running horizontally across the page, with the middle-branch hypotenuses $5, 29, 169, 985, 5741$ marked at their positions (not to scale — logarithmic spacing). Curved arrows arc above the number line, connecting successive pairs, each annotated with "$\times 6$" and "$-1\times$" (i.e., "$6c_n - c_{n-1}$"). The arrow from $29$ to $169$ is highlighted, with the computation $6 \times 29 - 5 = 169$ written below. The value $169 = 13^2$ is boxed and starred.]
...
Let us see why the answer is *never*, at least locally.
...
> **Theorem (Berggren–Hall).** Every primitive Pythagorean triple appears exactly once in the Berggren tree.
...
[ILLUSTRATION: Three subtrees of the Berggren tree drawn side by side, one for each branch (Left in red, Middle in blue, Right in green). All three are rooted at the same parent node $(3, 4, 5)$, shown in grey at the top. The subtrees' nodes are colored accordingly, and a prominent "no duplicates" symbol — a circle with a diagonal line through two overlapping shapes — hovers at the center where the three subtrees would intersect if they could. The visual message: the three families never intermarry.]
...
**The Setup.** You are given a composite number $N$ and asked to find a nontrivial factor — some $d$ with $1 < d < N$ that divides $N$. (This is the central problem of computational number theory, and the security of the internet depends on its difficulty for large $N$.) Here is the Pythagorean approach.

**Step 1.** Find a Pythagorean triple $(a, b, c)$ with $a = N$ — or, more generally, with $a$ sharing a nontrivial factor with $N$. The Berggren tree provides a structured source of triples.

**Step 2.** Compute $c - b$. By the difference-of-squares identity:

$$(c - b)(c + b) = a^2 = N^2.$$

**Step 3.** Compute $d = \gcd(c - b, \, N)$. If $1 < d < N$, you have found a nontrivial factor. Done.
...
**Theorem (GCD Factoring via Pythagorean Triples).** If $a^2 + b^2 = c^2$ with $a > 1$, and if $d = \gcd(c - b, \, a)$ satisfies $1 < d < |a|$, then $d$ is a nontrivial divisor of $a$.
...
**Worked example.** Take $N = 21$ and the triple $(21, 20, 29)$.
...
Wait — here $a = 33$, not $65$. We need $N$ to be a *leg*, not the hypotenuse. But the Berggren tree can also help us find triples with $c = N$ or with $a$ or $b$ sharing factors with $N$. The method is flexible.
...
[ILLUSTRATION: A flowchart-style diagram, elegant and vertical. **Input box** at the top: "A composite number $N$." **Arrow down** to **Step 1**: "Find a Pythagorean triple $(a, b, c)$ with $a = N$." The Berggren tree is sketched faintly in the background, suggesting the source of triples. **Arrow down** to **Step 2**: "Compute $c - b$ and $c + b$." Two numbers appear on cards. **Arrow down** to **Step 3**: "Compute $\gcd(c - b, N)$." A gear icon. **Arrow down** to a **diamond decision box**: "Is $1 < \gcd < N$?" **Yes arrow** leads to **Output**: "Nontrivial factor found! 🎉" **No arrow** loops back with the label "Try another triple."]

[ILLUSTRATION: The specific worked example for $(21, 20, 29)$, laid out as a visual "proof without words." At the top, the right triangle with sides $21$, $20$, $29$. Below it, a horizontal arrow from $29$ to $20$ labeled "$29 - 20 = 9$." Below that, a Venn-diagram-style overlap of the divisors of $9$ ($\{1, 3, 9\}$) and the divisors of $21$ ($\{1, 3, 7, 21\}$), with $\{1, 3\}$ in the overlap. The GCD $3$ is circled in red. At the bottom, the conclusion: $21 = 3 \times 7$, written in large, satisfying numerals.]

The connection to Fermat's factoring method is now clear. Fermat's approach, dating to the 1640s, searches for representations $N = x^2 - y^2 = (x - y)(x + y)$. The Pythagorean approach provides these representations *automatically* from the geometry of right triangles: every Pythagorean triple gives a free difference of squares, and the Berggren tree gives a systematic way to generate candidates. The modern quadratic sieve and number field sieve — the algorithms that actually break large numbers in practice — are, at a deep level, industrialized descendants of the same difference-of-squares principle. They generate many relations of the form $x^2 \equiv y^2 \pmod{N}$ and combine them using linear algebra over $\mathbb{F}_2$ to find a factor. The Berggren tree is the artisanal workshop; the quadratic sieve is the factory.

---

## Epilogue: Through the Wormhole
...
1. **The Ancient Identity.** The humble equation $(c - b)(c + b) = a^2$ — a one-line consequence of the Pythagorean theorem — turned out to be a factoring engine. (§1)

2. **The Berggren Tree.** Three $3 \times 3$ matrices, discovered by a Swedish schoolteacher in 1934, generate every primitive Pythagorean triple from the single seed $(3, 4, 5)$. (§2)

3. **The Lorentz Connection.** The Berggren matrices are integer Lorentz transformations — they preserve the quadratic form $Q(a, b, c) = a^2 + b^2 - c^2$, which is the equation of a light cone in Minkowski spacetime. Pythagorean triples are null vectors, and the tree lives on the surface of the cone. (§3–§4)

4. **Paths and Shortcuts.** Every triple has a unique address in the tree, and the address can be traversed in logarithmic time using repeated squaring. Inverse matrices — computed via the Lorentz adjoint — let you climb back up from any node to the root. (§5–§7)

5. **Structure and Growth.** The hypotenuse always increases as you descend, with the middle branch growing fastest (governed by a Chebyshev recurrence). No two branches ever produce the same triple. And the difference-of-squares identity, combined with a GCD computation, turns any triple into a factoring tool. (§8–§11)
...
In the next chapter, we will shift perspective entirely. If the Berggren tree generates all primitive Pythagorean triples by matrix multiplication, can we understand the same triples through a completely different lens — the lens of *Gaussian integers*, where the Pythagorean equation dissolves into the norm of a complex number? The answer will bring together three seemingly independent roads — parametric, geometric, and algebraic — and reveal that they converge at a single crossroads.

But that is a story for another day.

[ILLUSTRATION: A full-page "map" of the chapter's conceptual landscape, drawn in the style of a medieval mappa mundi or a fantasy-novel treasure map. The map contains labeled regions: "The Broken Square" (a cracked stone tablet near the coast), "Berggren Forest" (a dense ternary forest of branching paths), "The Light Cone" (a mountainous volcanic peak with conical sides and lattice points glowing on its slopes), "Shortcut Wormhole" (a swirling portal connecting two distant points), "The Elevator" (a mineshaft with upward arrows), "Chebyshev Ridge" (a series of peaks growing exponentially taller), and "The Factoring Forge" (a smithy where numbers are being split on an anvil). Paths connect these regions, retracing the chapter's narrative arc. A compass rose in the corner points not to N/S/E/W but to "Algebra / Geometry / Analysis / Number Theory." The map is bordered with decorative flourishes and tiny Pythagorean triples hidden in the marginalia.]

---

### Puzzles for the Reader

> **Puzzle 1.** The triple $(8, 15, 17)$ is primitive and Pythagorean. Compute $c - b$ and $\gcd(c - b, a)$. Does the GCD reveal a nontrivial factor of $8$?

> **Puzzle 2.** Find the address of the triple $(7, 24, 25)$ in the Berggren tree by ascending from $(7, 24, 25)$ to the root using inverse matrices. (Hint: try each $B_i^{-1}$ at each step and check which one gives all-positive entries.)

> **Puzzle 3.** Verify the Chebyshev recurrence $c_{n+1} = 6c_n - c_{n-1}$ for the *legs* of the middle branch, not just the hypotenuses. Do the $a$-values and $b$-values of the middle branch satisfy the same recurrence?

> **Puzzle 4.** Compute $B_2^2$ explicitly and verify that it sends $(3, 4, 5)$ to $(119, 120, 169)$. Then verify that $169 = 13^2$, and use the difference-of-squares trick to factor the leg $a = 119$.

> **Puzzle 5.** The Lorentz adjoint formula says $B_i^{-1} = \mathbf{Q} B_i^\top \mathbf{Q}$. Verify this for $B_3$ by computing both sides explicitly.

> **Puzzle 6.** How many matrix multiplications does it take to compute $B_2^{64}$ by repeated squaring? How about $B_2^{100}$?
