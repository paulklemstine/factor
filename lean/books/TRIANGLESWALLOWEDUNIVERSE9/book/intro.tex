...
\caption{A large, beautifully rendered depiction of a right triangle with legs labeled $a = 3$ and $b = 4$ and hypotenuse $c = 5$. On each side of the triangle, a perfect square grid is constructed outward: a\ldots}
\end{figure}
...
\begin{quote}
\itshape \textbf{Can you ever run out?} Is there a largest Pythagorean triple, after which no new ones exist? Or do they go on forever? And if they go on forever --- is there a \textit{pattern}? A system? A map?
\end{quote}
...
\caption{A table of the first sixteen primitive Pythagorean triples, organized in four columns. Column 1: the triple $(a, b, c)$ with $a$ odd and $b$ even. Column 2: the Euclid parameters $(m, n)$ that\ldots}
\end{figure}
...
\begin{quote}
\itshape \textbf{Euclid's Classification.} Every primitive Pythagorean triple $(a, b, c)$ with $a$ odd and $b$ even is given by $$a = m^2 - n^2, \qquad b = 2mn, \qquad c = m^2 + n^2$$ where $m > n > 0$, $\gcd(m, n) = 1$, and $m - n$ is odd. Moreover, different valid pairs $(m, n)$ always produce different triples.
\end{quote}
...
\caption{A whimsical, full-page scene depicting a person sitting on the edge of a bathtub, staring at a bathroom floor tiled with white unit squares. On the floor, three regions of tiles are outlined in\ldots}
\end{figure}
