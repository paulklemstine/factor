...
\caption{A single right triangle with legs labeled $N$ and $(N^2-1)/2$ and hypotenuse $(N^2+1)/2$. Beside it, the seven-row table above is rendered as a neatly ruled card. The triangle for $N = 15$ is drawn\ldots}
\end{figure}
...
\caption{A visual "crowbar" diagram. On the left, a right triangle with sides labeled $N = 15$, $b$, $c$. An arrow labeled "subtract" points to the equation $(c-b)(c+b) = N^2$. On the right, the number\ldots}
\end{figure}
...
\caption{A ternary tree diagram, three levels deep, rooted at $(3,4,5)$ at the top. Three branches descend to the first-generation children: $(5,12,13)$, $(21,20,29)$, $(15,8,17)$. Each of these branches into\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter14/images/fig04_reverse_climb.png}
\caption{A small inset panel beside the tree. A single triple at a leaf node has an arrow pointing upward, labeled "$B^{-1}$", tracing the unique path back to the root $(3,4,5)$. Caption: "Climbing the tree:\ldots}
\end{figure}
...
\caption{A worked example shown as a vertical "ladder." Start at the bottom with the triple $(39, 80, 89)$. Apply $B_2^{-1}$: an upward arrow points to the parent $(21, 20, 29)$. Apply $B_2^{-1}$ again:\ldots}
\end{figure}
...
\caption{A graph with the horizontal axis labeled "descent step" (showing values $0, 1, 2, 3, \ldots$) and the vertical axis labeled "hypotenuse $c$." A descending staircase of bold dots shows the hypotenuse\ldots}
\end{figure}
...
\caption{A triple $(a, b, c)$ displayed in a central circle. Three arrows radiate outward to three boxes labeled $B_1^{-1}(a,b,c)$, $B_2^{-1}(a,b,c)$, and $B_3^{-1}(a,b,c)$. Two of the boxes are shaded red\ldots}
\end{figure}
...
\caption{A flowchart of the factoring algorithm. Start: "Input odd $N$." Box 1: "Compute trivial triple $(N, \frac{N^2-1}{2}, \frac{N^2+1}{2})$." Diamond: "Is $\gcd(a, N)$ or $\gcd(b, N)$ non-trivial?" If yes\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter14/images/fig09_worked_77.png}
\caption{A detailed worked example for $N = 77 = 7 \times 11$. A vertical sequence of triples is shown, each connected by an upward arrow. At each step, the values $\gcd(a_i, 77)$ and $\gcd(b_i, 77)$ are\ldots}
\end{figure}
...
Henri Poincar\'e discovered, in the 1880s, that the interior of such a cone is a model of \textit{hyperbolic geometry} --- the strange non-Euclidean geometry where parallel lines diverge and triangles have angle sums less than $180?$. The Berggren tree, from this vantage point, is a discrete tiling of the hyperbolic plane by ideal triangles. But that is a story for another chapter.
\index{hyperbolic geometry}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter14/images/fig10_light_cone.png}
\caption{A three-dimensional wireframe cone, vertex at the origin, with the vertical axis labeled $c$ and the horizontal plane spanned by axes $a$ and $b$. The cone's surface is the locus $a^2 + b^2 = c^2$.\ldots}
\end{figure}
...
\caption{Five side-by-side "descent ladders," one for each of the five examples. Each ladder is a vertical chain of small boxes containing triples, connected by upward arrows. GCD values are annotated at each\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter14/images/fig12_bar_chart.png}
\caption{A bar chart comparing "number of descent steps" (vertical axis) versus $N$ (horizontal axis, showing $15, 21, 77, 143, 323$). The bars increase in height. A faint dashed curve proportional to\ldots}
\end{figure}
...
\begin{enumerate}
\item **Every odd $N$ has a trivial Pythagorean triple.** The identity $N^2 + \left(\frac{N^2-1}{2}\right)^2 = \left(\frac{N^2+1}{2}\right)^2$ hands us a starting point for free.
\item **Pythagorean triples biject with same-parity divisor pairs of $N^2$.** The difference-of-squares crowbar $(c-b)(c+b) = N^2$ converts between triples and factorizations.
\item **The Berggren tree organizes all primitive triples into a single ternary tree rooted at $(3,4,5)$.** Three transformation rules, discovered in 1934, generate every primitive triple exactly once.
...
\item **GCDs of leg values with $N$ may reveal non-trivial factors.** The Berggren maps, by linearly recombining the components, "stir" the arithmetic until a hidden factor precipitates.
\end{enumerate}
...
\item \textit{Is there a connection to lattice reduction?} The Berggren tree, viewed as a set of integer points on the light cone, is intimately related to shortest vectors in certain lattices. The Lattice-Tree Correspondence is developed in Chapter 2.
\end{itemize}
...
\caption{A grand, full-page composite image. At the center, a large ternary tree grows downward from $(3,4,5)$ at the top, extending four or five levels deep, its branches carrying labeled triples at every\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter14/images/fig14_book_map.png}
\caption{A whimsical "map of the book" rendered in the style of a medieval cartographer's chart, with Chapter 14 drawn as a central island. Bridges radiate outward to neighboring islands labeled Chapter 2\ldots}
\end{figure}
