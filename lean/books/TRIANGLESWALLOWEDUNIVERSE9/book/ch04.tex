...
\begin{center}\large\itshape \textit{How the World's Oldest Equation Secretly Cracks the World's Hardest Codes}\end{center}
\medskip
\label{ch04}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig02_gaussian_lattice.png}
\caption{The Gaussian integer lattice $\mathbb{Z}[i]$ in the complex plane. Points $1+2i$ and $2+3i$ are shown as vectors from the origin, drawn in contrasting colors. Their product $-4+7i$ is shown as a\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig03_triple_factory.png}
\caption{A whimsical "Triple Factory" conveyor belt diagram. Two input conveyor belts enter from the left, one carrying a card labeled $(3, 4, 5)$ with a small right triangle drawn on it, the other carrying\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig04_multiplication_table.png}
\caption{A multiplication table of small Pythagorean triples. Rows and columns are headed by $(3,4,5)$, $(5,12,13)$, $(8,15,17)$, and $(7,24,25)$. Each cell shows the composed triple (using the first variant\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig05_euler_xray.png}
\caption{"Euler's X-Ray Machine." A large number $N = 1105$ sits inside an ornate picture frame. On either side, two X-ray screens display the number's inner structure: the left screen shows\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig06_euler_portrait.png}
\caption{A portrait sketch of Leonhard Euler at his desk, quill in hand, late in life (one eye clouded over). A thought bubble above his head contains the factoring identity $(a - c)(a + c) = (d - b)(d + b)$.\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig07_pythagorean_cone.png}
\caption{A three-dimensional perspective drawing of the cone $x^2 + y^2 = z^2$ rising upward from the origin. The surface is translucent, revealing a grid of integer lattice points. The point $(3, 4, 5)$ is\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig08_light_cones.png}
\caption{A side-by-side comparison. LEFT: A relativistic light cone in $2+1$ spacetime dimensions, with a photon worldline traced along the cone's surface and labels for "future," "past," and "elsewhere."\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig09_berggren_tree.png}
\caption{A full ternary tree, three levels deep. The root node at the top contains $(3, 4, 5)$ with a small right triangle drawn to scale. Three branches descend, colored blue ($B_1$), red ($B_2$), and green\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig10_tree_sieve.png}
\caption{A stylized tree with thick trunk and spreading branches. Some branches bear glowing "fruit" --- small spheres labeled with divisor values. At the base of the tree, a figure in a mathematician's robe\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig11_divisor_arcs.png}
\caption{A number line showing $N^2 = 31861^2$ with tick marks at selected divisor pairs. Arcs connect complementary pairs $(c - b, c + b)$ that multiply to $N^2$. Stars mark the positions where arcs reveal\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig12_three_roads_map.png}
\caption{A stylized map drawn in the manner of a medieval cartographer. Three winding roads converge on a fortified castle at the center, labeled "$p$ and $q$." Road 1, "Euler's Two Portraits," passes through\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig13_exponential_climb.png}
\caption{A vertical logarithmic scale running from $10^0$ at the bottom to $10^{20}$ at the top. The Berggren tree is drawn sideways, with the root at the left and branches extending to the right. The\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig14_flower_diagram.png}
\caption{A "flower" diagram. At the center, a circle contains $N^2$. Four petals radiate outward, each containing one sum-of-squares representation of $N^2$. The top two petals are labeled "parent\ldots}
\end{figure}
...
Tree Sieve & Berggren tree + target $N$ & Smooth relations \ensuremath{\to} factors & $(c - b)(c + b) = N^2$, then $\gcd$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig15_factoring_toolkit.png}
\caption{A "Pythagorean Factoring Toolkit" infographic divided into three columns. Column 1 (amber): "Euler's Two Portraits" --- the factoring identity, a worked example with $N = 1105$, and a small portrait of\ldots}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter4/images/fig16_road_ahead.png}
\caption{A winding road receding into a misty distance. Mile markers along the road are labeled with the topics of future chapters: "Lattice Reduction" (Chapter 5), "Complexity Bounds" (Chapter 6), "Quantum\ldots}
\end{figure}

But before you turn the page, here is a parting puzzle to test your mastery of the three roads.

\begin{quote}
\itshape \textbf{Closing Puzzle.} The number $N = 5{,}525$ can be written as a sum of two squares in at least three different ways. Find all three representations, and use Euler's method --- applied to each pair --- to recover the complete prime factorization $N = 5^2 \times 13 \times 17$.
\end{quote}
...
\begin{quote}
\itshape \textbf{Puzzle 1.} Verify the Brahmagupta--Fibonacci identity for $a = 3$, $b = 4$, $c = 5$, $d = 12$. What two representations of $5^2 \cdot 13^2 = 4225$ do you obtain?
\end{quote}

\begin{quote}
\itshape \textbf{Puzzle 2.} Compose the Pythagorean triples $(8, 15, 17)$ and $(7, 24, 25)$ using the composition theorem. Verify that the result is Pythagorean with hypotenuse $17 \times 25 = 425$.
\end{quote}

\begin{quote}
\itshape \textbf{Puzzle 3.} The number $901$ can be written as $1^2 + 30^2$ and as $10^2 + \sqrt{901 - 100}^2$... but can it? Determine whether $901$ has two sum-of-squares representations, and if so, use Euler's method to factor it.
\end{quote}

\begin{quote}
\itshape \textbf{Puzzle 4.} Apply all three Berggren transformations to $(5, 12, 13)$ and verify that the three children all satisfy $Q = 0$.
\end{quote}

\begin{quote}
\itshape \textbf{Puzzle 5.} Starting from the hypotenuse growth lemma $c' \geq 3c$, estimate how many generations of the Berggren tree it takes to reach a triple with hypotenuse exceeding one million. Then compute the exact number of nodes you would need to search at that depth.
\end{quote}
