...
/-- The generalized Lorentz form Q_{n,1}: sum of first n squares minus last square.
    For a vector v : Fin (n+1) -> Int, Q(v) = v_02 + v_12 + ... + v_{n-1}2 - v_n2. -/
...
/-- A Pythagorean quintuplet (a,b,c,d,e) satisfies a2 + b2 + c2 + d2 = e2. -/
...
/-- A Pythagorean sextuplet (a,b,c,d,e,f) satisfies a2 + b2 + c2 + d2 + e2 = f2. -/
...
/-- A Pythagorean octuplet (a_1,...,a_7,a_8) satisfies suma_i2 = a_82. -/
...
/-- The inside-out parametrization for quadruples: given N, parameters u and v,
    compute the hypotenuse h2 = N2 + u2 + v2. -/
def insideOutQuadHyp (N u v : Int) : Int := N ^ 2 + u ^ 2 + v ^ 2

/-- The inside-out triple: N2 + u2 = h2 gives (h-u)(h+u) = N2. -/
theorem inside_out_triple_factor (N u h : Int) (hp : N ^ 2 + u ^ 2 = h ^ 2) :
    (h - u) * (h + u) = N ^ 2 := by nlinarith

/-- **Inside-Out Quadruple Factor Theorem**: Given N2 + u2 + v2 = h2,
    we have (h-v)(h+v) = N2 + u2, providing a sum-of-squares decomposition
    of N2 + u2 that may reveal factors. -/
...
Each representation a2 + b2 = N gives a Pythagorean triple (a, b, N),
yielding a factoring channel via gcd(N-b, N).
-/

/-- **Sum-of-two-squares factor theorem**: If N2 - c2 = (N-c)(N+c) and
...
The squaring map x ? x2 mod N generates orbits. If x2 === y2 (mod N) with
...
**Congruence of squares -> factoring**: The classical bridge.
    If N | (x2-y2) but N ? (x-y) and N ? (x+y), then gcd(x-y, N) > 1.
-/
...
Starting from a node (a_1, ..., a_{k-1}, N) with large N, we descend toward
...
/-- Inverse Berggren transform B_2^-1 -/
def invB2' (a b c : Int) : Int x Int x Int :=
  (a + 2*b - 2*c, 2*a + b - 2*c, -2*a - 2*b + 3*c)

/-- B_2^-1 preserves Pythagorean property. -/
...
Given a triple (a,b,c) and any integer d, we get a quintuplet
  (a, b, 0, d, e)
whenever c2 + d2 = e2 is a perfect square.
-/
...
/-- **Nontrivial lifting**: A triple (a,b,c) + integer d with c2+d2 = e2
...
### Theorem (R_1_1_1_1 Descent Factor Extraction)

The reflection R_1_1_1_1(a,b,c,d) = (d-b-c, d-a-c, d-a-b, 2d-a-b-c) preserves
...
/-- The R_1_1_1_1 reflection for quadruples -/
def reflect1111 (a b c d : Int) : Int x Int x Int x Int :=
  (d - b - c, d - a - c, d - a - b, 2*d - a - b - c)

/-- R_1_1_1_1 preserves the quadruple equation. -/
...
Under R_1_1_1_1 descent, when a+b+c > d (which holds for positive components on
the null cone by Cauchy-Schwarz), the reflected hypotenuse strictly decreases.
-/

/-- The L1 energy of a quadruple. -/
...
theorem descent_energy_hyp_decrease (a b c d : Int)
  ...
  -- From h: d2 = a2 + b2 + c2 < (a+b+c)2, so d < a+b+c
...
The Brahmagupta-Fibonacci identity (a2+b2)(c2+d2) = (ac-bd)2 + (ad+bc)2
...
theorem euler_four_square (a_1 a_2 a_3 a_4 b_1 b_2 b_3 b_4 : Int) :
    (a_1^2 + a_2^2 + a_3^2 + a_4^2) * (b_1^2 + b_2^2 + b_3^2 + b_4^2) =
    (a_1*b_1 - a_2*b_2 - a_3*b_3 - a_4*b_4)^2 +
    (a_1*b_2 + a_2*b_1 + a_3*b_4 - a_4*b_3)^2 +
    (a_1*b_3 - a_2*b_4 + a_3*b_1 + a_4*b_2)^2 +
    (a_1*b_4 + a_2*b_3 - a_3*b_2 + a_4*b_1)^2 := by ring

/-- **Tuple multiplication for factoring**: If N_1 and N_2 are sums of k squares,
    then N_1.N_2 is also a sum of k squares (for k = 2, 4).
    This allows composing factoring channels. -/
theorem compose_factoring_channels (a b c d : Int) (N_1 N_2 : Int)
    (h1 : a ^ 2 + b ^ 2 = N_1) (h2 : c ^ 2 + d ^ 2 = N_2) :
    (a*c - b*d) ^ 2 + (a*d + b*c) ^ 2 = N_1 * N_2 := by
  rw [<- h1, <- h2]; ring
...
A Pythagorean octuplet (a_1,...,a_7,N) with N composite provides 7 primary channels
...
/-- Verify: (1,2,3,4,5,6,3) gives octuplet with sum = 100 = 102. -/
...
sphere of radius N in Real3, which is 4piN2. But only O(N^{1+epsilon}) of these
...
(N-1)2 + 1 + 1 = N2 - 2N + 3. This equals N2 iff N = 3/2, so we need
...
/-- **Lagrange connection**: If N2 is expressed as a_12 + a_22 + a_32 + a_42
...
/-- **Quadratic residue bridge**: x2 mod N can be decomposed as a sum of squares
...
5. `reflect1111_preserves` -- R_1_1_1_1 preserves null cone
