...
Actually, Fermat proved the stronger result: a? + b? = c2 has no
positive integer solutions. This immediately implies FLT for n = 4
since a? + b? = c? would give a? + b? = (c2)2.

**Proof sketch (infinite descent):**
Suppose a? + b? = c2 with a, b, c > 0 and c minimal.
Then (a2, b2, c) is a Pythagorean-like triple. Factor and descend to
...
/-
PROBLEM
The stronger form: a? + b? != c2 for positive integers.
...
FLT for n = 3: a3 + b3 != c3 for positive integers.
...
1. From a? + b? = c?, factor the left side:
   (a + b)(a + zetab)(a + zeta2b)...(a + zeta?^-1b) = c?
