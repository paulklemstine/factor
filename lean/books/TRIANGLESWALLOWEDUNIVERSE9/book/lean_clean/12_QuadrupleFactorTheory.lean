...
a2 + b2 + c2 = d2 to the problem of integer factoring. The key insight is that
...
1. **Difference-of-Squares Decomposition**: d2 - c2 = a2 + b2 gives (d-c)(d+c) = a2 + b2,
   linking quadruples to factorizations.
2. **GCD Factor Extraction**: gcd(d-c, a2+b2) divides both d-c and d+c, yielding factors.
...
4. **Parametric Factor Revelation**: The (m,n,p,q) parametrization reveals
   d = (m2+n2) + (p2+q2), decomposing d into sum-of-two-squares components.
...
/-- A Pythagorean quadruple is a 4-tuple (a,b,c,d) with a2 + b2 + c2 = d2. -/
...
/-- **Core Factoring Identity**: For any Pythagorean quadruple,
    (d - c)(d + c) = a2 + b2. This bridges quadruples to factoring. -/
...
/-- **Parametric Factor Revelation**: In the parametrization,
    d = (m2 + n2) + (p2 + q2), decomposing d as a sum of two
...
/-- **Parametric a2 + b2 factorization**: a2 + b2 = (d-c)(d+c). -/
...
/-
**Sum-of-Squares Collision Principle**: If N = a_12 + b_12 + c_12 = a_22 + b_22 + c_22
    with different c values, then (c_12-c_22) = (a_22-a_12) + (b_22-b_12).
-/
...
/-- The norm-squared of a Gaussian integer z = a + bi is a2 + b2. -/
...
/-- **Gaussian Factoring Principle**: a2 + b2 = d2 - c2. -/
...
/-
**Prime Divisor Dichotomy**: If p is prime and p | a2+b2 = (d-c)(d+c),
...
/-- **Mod 8 Structure**: When all components are even, 8 divides d2-a2-b2-c2. -/
