...
Q(a, b, c) = a2 + b2 - c2, establishing them as elements of the
...
/-- Berggren matrix A preserves the Lorentz form.
    If (a,b,c) satisfies a2 + b2 = c2, then so does
...
/-- The Lorentz form Q(a,b,c) = a2+b2-c2 is preserved by all three
...
/-- For the consecutive-parameter case (n = m-1), the Berggren depth
    ...
    (2m-1, 2m(m-1), 2m2-2m+1) has depth m-2 in the Berggren tree,
...
/-- The counting theorem for semiprimes: n = p x q has exactly 4 Pythagorean triples.
    This is because sigma_0(n2) = sigma_0(p2q2) = 3 x 3 = 9, so |T(n)| = (9-1)/2 = 4. -/
theorem semiprime_four_triples (p q : Nat) (_hp : Nat.Prime p) (_hq : Nat.Prime q)
    ...
    -- The divisor pairs are: (1, p2q2), (p, pq2), (q, qp2), (p2, q2)
    let n := p * q
    1 * (n ^ 2) = n ^ 2 /\
    p * (p * q ^ 2) = n ^ 2 /\
    q * (q * p ^ 2) = n ^ 2 /\
