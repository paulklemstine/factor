...
is Theta(?N) for balanced semiprimes N = p.q with p ~= q.
-/
...
/-- **Main complexity theorem**: For a balanced semiprime N = p.q,
    ...
    Total: O(?N . log N) bit operations = Theta(?N) arithmetic operations. -/
theorem pythagorean_tree_complexity (N p q : Nat)
    (hN : N = p * q) (_hp : 2 <= p) (hpq : p <= q) :
    p * p <= N := by
  subst hN; exact Nat.mul_le_mul_left p hpq

/-- **Lower bound**: Tree factoring cannot do better than Theta(?N). -/
