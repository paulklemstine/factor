...
/-- Each channel value equals a difference of squares. Channel for (b,c): b2+c2 = d2-a2. -/
...
/-- If g divides two channel values (a2+b2) and (a2+c2), then g divides b2-c2. -/
...
/-- If g divides all three channels, it divides 2a2, 2b2, 2c2. -/
...
/-- If p | d and p | c, then p2 divides the channel a2+b2. -/
...
/-- d2 mod 4 is 0 or 1, so a2+b2+c2 mod 4 is 0 or 1. -/
...
/-- If d is even, then a2+b2+c2 is divisible by 4. -/
...
/-- Channel product via d: (a2+b2)(a2+c2) = a2d2 + b2c2. -/
...
/-- Full channel product = (b2+c2)(a2+c2)(a2+b2). -/
...
/-- If p | d and p | c_1, then p2 | (a_12+b_12). -/
...
/-- If p | a, p | b, p | c in a quadruple, then p2 | d2 and we can descend. -/
...
/-- Sextuple (5 spatial components): 10 pair channels sum to 4f2. -/
...
/-- Septuple (6 spatial components): 15 pair channels sum to 5g2. -/
...
/-
No nonzero balanced quadruple: 3a2 != d2 for a != 0.
-/
...
/-- If a2+b2 = (2k+m).m for some integers, we get a quadruple (a,b,k,k+m). -/
...
/-- If 0 < d-c, then d-c divides a2+b2 and d-c < d (for c > 0). -/
...
-- Ch(a,b) = 62+102 = 136 = 8x17
-- Ch(a,c) = 62+332 = 1125 = 53x9
-- Ch(b,c) = 102+332 = 1189 = 29x41
...
-- Q = (6, 9, 18, 21)? 36+81+324 = 441 = 212 ?
example : (6:Int)^2 + 9^2 + 18^2 = 21^2 := by norm_num
-- Channel: (21-18)(21+18) = 3x39 = 117 = 62+92
