...
1. **`trivial_triple_is_pyth`**: For any odd N, (N, (N2-1)/2, (N2+1)/2) is Pythagorean
2. **`diff_of_squares`**: N2 + b2 = c2  ?  (c-b)(c+b) = N2
...
/-- For any odd N, the triple (N, (N2-1)/2, (N2+1)/2) satisfies the Pythagorean equation. -/
...
/-- The core algebraic identity: if N2 + b2 = c2 then (c-b)(c+b) = N2. -/
...
/-- B_1^-1 comp B_1 = Id (component-wise) -/
...
/-- B_2^-1 comp B_2 = Id (component-wise) -/
...
/-- B_3^-1 comp B_3 = Id (component-wise) -/
