...
1. **Brahmagupta-Fibonacci identity**: (a2+b2)(c2+d2) = (ac-bd)2 + (ad+bc)2
...
/-- If (a_1, b_1, c_1) and (a_2, b_2, c_2) are Pythagorean triples
    (a2 + b2 = c2), then their Gaussian composition is also Pythagorean.
    Specifically, (a_1a_2 - b_1b_2, a_1b_2 + b_1a_2, c_1c_2) is Pythagorean. -/
theorem pythagorean_composition
    (a_1 b_1 c_1 a_2 b_2 c_2 : Int)
    (h_1 : a_1 ^ 2 + b_1 ^ 2 = c_1 ^ 2)
    (h_2 : a_2 ^ 2 + b_2 ^ 2 = c_2 ^ 2) :
    (a_1 * a_2 - b_1 * b_2) ^ 2 + (a_1 * b_2 + b_1 * a_2) ^ 2 = (c_1 * c_2) ^ 2 := by
  nlinarith [brahmagupta_fibonacci a_1 b_1 a_2 b_2]
...
/-- If N = a2 + b2 = c2 + d2 with (a,b) != (c,d) and (a,b) != (d,c),
    then gcd(a-c, b-d) or gcd(a+c, b-d) gives information about factors.
    
    More precisely: if a2+b2 = c2+d2 then (a2-c2) = (d2-b2),
    i.e., (a-c)(a+c) = (d-b)(d+b). -/
...
/-- The difference of squares form: if N = a2+b2 = c2+d2 then
    N2 = (ac+bd)2 + (ad-bc)2 = (ac-bd)2 + (ad+bc)2. -/
...
The Berggren matrices preserve the quadratic form Q(x,y,z) = x2 + y2 - z2.
This section proves properties of this form relevant to the tree sieve. -/

/-- The Lorentz form Q(a,b,c) = a2 + b2 - c2. For Pythagorean triples, Q = 0. -/
...
/-- If (c-b)(c+b) = N2 with c > b > 0, then each of c-b and c+b
    is a divisor of N2. This is the foundation of the tree sieve's
...
/-- The GCD extraction: if d divides N2 and 1 < gcd(d, N) < N,
...
/-- The product ab for a Pythagorean triple satisfies 2ab <= c2.
...
/-- For a Pythagorean triple, 2.a.b <= a2 + b2. -/
...
/-- For a semiprime N = p*q with p, q odd primes, the number N2
...
/-- The two factorizations of N2 give different divisor pairs when p != q. -/
...
/-- The B_2 child has hypotenuse 2a + 2b + 3c >= 3c when a, b >= 0. -/
theorem hypotenuse_growth_B2 (a b c : Int) (ha : 0 <= a) (hb : 0 <= b) :
    2 * a + 2 * b + 3 * c >= 3 * c := by
  linarith

/-- After k applications of B_2, hypotenuse >= 3^k . c_0. -/
theorem hypotenuse_exponential (c_0 : Int) (k : Nat) (hc : 0 < c_0) :
    (3 : Int) ^ k * c_0 > 0 := by
  positivity
