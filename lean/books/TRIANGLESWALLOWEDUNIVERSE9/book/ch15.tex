\chapter{\textit{The Algebra Where Two Plus Three Equals Two: Tropical Geometry and the Shortest-Path Semiring}}
\label{ch15}
...
\caption{A whimsical blueprint-style diagram of a calculator keypad. The "$+$" key is relabeled "$\min$" in tropical-orange ink, and the "$\times$" key is relabeled "$+$". Beside the calculator, a small table\ldots}
\end{figure}
...
\caption{A number line running from $0$ to $15$. Two points are marked at $b = 7$ and $c = 2$, with an arc labeled "$\oplus$" pointing to $2$ (the minimum). Then the entire line is shifted right by $a = 4$\ldots}
\end{figure}
...
\caption{A vertical "totem pole" of integers from $-3$ up to $+5$, with a cloud at the very top labeled "$\infty = $ tropical zero". An arrow from any integer $a$ to the cloud is labeled "$\oplus$", and the\ldots}
\end{figure}
...
\caption{A coordinate plane with horizontal axis labeled "exponent $i$" and vertical axis labeled "valuation $a_i$". Three points are plotted: $(0, 2)$, $(1, 7)$, $(2, 3)$. The lower convex hull is drawn as\ldots}
\end{figure}
...
\caption{Two side-by-side panels. LEFT: Classical convex hull --- a shaded triangle with vertices $A$, $B$, $C$ in $\mathbb{R}^2$, showing the line segment from $A$ to $B$ and the interior. RIGHT: Tropical\ldots}
\end{figure}
...
\caption{Three cities ($x$, $y$, $z$) arranged on a road map, with roads of integer length connecting them. The direct road from $x$ to $z$ has length $d(x,z) = 5$, while the detour through $y$ has total\ldots}
\end{figure}
...
\caption{A $4 \times 4$ weighted directed graph with cities labeled $A$, $B$, $C$, $D$. Edge weights are single-digit integers. Beside the graph, two $4 \times 4$ matrices: the adjacency matrix $D$ (with\ldots}
\end{figure}
...
\caption{A five-vertex directed graph with source vertex $0$ at left. Arrows show edges with integer weights. Below the graph, a row of boxes showing $d(v)$ values after each relaxation round: Round 0 shows\ldots}
\end{figure}
...
\caption{A large "map of the book" diagram, drawn as a network graph. Each chapter is a node (labeled $1$ through $16$), and directed edges connect chapters whose concepts feed into later ones. The path from\ldots}
\end{figure}
...
\caption{A piecewise-linear graph of the tropical polynomial $p(x) = \min(3,\; 1+x,\; 4+2x)$, drawn on a standard $xy$-plane. Three line segments of different slopes meet at two "kink" points (the tropical\ldots}
\end{figure}
