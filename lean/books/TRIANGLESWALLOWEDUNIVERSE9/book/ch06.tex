...
\begin{center}\large\itshape \textit{How Pythagorean Quintuplets, Sextuplets, and Octuplets Crack Composite Numbers Wide Open}\end{center}
\medskip
\label{ch06}
...
\begin{quote}
\itshape *Given a composite number $N$, how many independent "keyholes" can you create from a single equation?*
\end{quote}
...
\caption{A large ornate bank vault door with seven distinct keyholes arranged in a circle. Each keyhole is labeled with a variable: $a_1, a_2, \ldots, a_7$. In the center of the door is inscribed "$N$". Below\ldots}
\end{figure}
...
\caption{A "phylogenetic tree" of Pythagorean creatures, drawn whimsically like a naturalist's evolutionary diagram. At the bottom is the classic triple $(3,4,5)$ labeled "The Ancestor," drawn as a sturdy\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter6/images/fig03_specimen_catalog.png}
\caption{A specimen catalog table, styled with elegant borders like a 19th-century naturalist's journal. Three rows for quintuplets: $(1,1,1,1,2)$, $(1,2,2,4,5)$, $(1,4,4,4,7)$; two rows for sextuplets:\ldots}
\end{figure}
...
\begin{quote}
\itshape \textbf{The Multi-Channel Factor Extraction Theorem.} Let $a_1^2 + a_2^2 + \cdots + a_{k-1}^2 = N^2$ be a Pythagorean $k$-tuple with hypotenuse $N$. For each spatial component $a_i$, we have  $$(N - a_i)(N + a_i) = \sum_{j \ne i} a_j^2,$$  and therefore $\gcd(N - a_i, \, N)$ divides $N$. If $1 < \gcd(N - a_i, \, N) < N$ for any $i$, then $N$ has a nontrivial factor.
\end{quote}
...
\caption{A circular "combination lock" diagram for $N = 15$. The outer ring shows the quadruple $(5, 10, 10, 15)$ with the three spatial components placed at equally-spaced points on the circle. Three arrows\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter6/images/fig05_lock_N21.png}
\caption{A similar lock diagram for $N = 21$, with the quadruple $(6, 9, 18, 21)$ and three channels shown: $\gcd(15, 21) = 3$, $\gcd(12, 21) = 3$, $\gcd(3, 21) = 3$. The winning factor $3$ is highlighted,\ldots}
\end{figure}
...
\item **Total: $28$ independent factoring attempts from a single equation.**
\end{itemize}
...
\caption{A complete graph $K_7$ (seven vertices, all $21$ edges drawn) superimposed on a regular heptagonal frame. Each vertex is labeled $a_1$ through $a_7$ and drawn as a small keyhole shape. Each edge is\ldots}
\end{figure}
...
\begin{quote}
\itshape \textbf{The Dimension Advantage Theorem.} In dimension $k$, the inside-out method has $k - 2$ free parameters $u_1, \ldots, u_{k-2}$. Each parameter provides an independent factoring channel. More dimensions, more freedom, more chances.
\end{quote}
...
\caption{Three panels side by side, showing inside-out factoring in increasing dimensions. Panel 1 (Triple): A 2D cross-section of a cone, with a single point labeled $N$ on the axis and one free parameter\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter6/images/fig08_maze.png}
\caption{A maze shown from above, with a single winding path from START to FINISH. The start end is labeled "Search for tuples" and is colored in frustration-red. Next to it, the same maze viewed from the\ldots}
\end{figure}
...
\caption{A rooted tree diagram with $(3, 4, 5)$ at the root. Three branches ascend to children: $(5, 12, 13)$, $(21, 20, 29)$, $(15, 8, 17)$. Each child has three children of its own, and so on for three\ldots}
\end{figure}
...
\caption{A "before and after" pair of Pythagorean quadruples, arranged as two tetrahedra. The left tetrahedron has vertices labeled $a$, $b$, $c$ (spatial, at the base) and $d$ (hypotenuse, at the apex). The\ldots}
\end{figure}
...
\caption{A cutaway building with floors labeled 3 through 8, one above the other. An elevator shaft runs through the middle. On Floor 3, the triple $(3, 4, 5)$ sits in a small room, drawn as a right triangle.\ldots}
\end{figure}
...
\caption{Two gears meshing together. The left gear has four teeth labeled "$a$", "$b$", "$a^2$", "$b^2$"; the right gear has four teeth labeled "$c$", "$d$", "$c^2$", "$d^2$". The output shaft below is\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter6/images/fig13_euler_table.png}
\caption{A $4 \times 4$ grid showing the Euler four-square multiplication table. The rows are labeled $b_1, b_2, b_3, b_4$ and the columns $a_1, a_2, a_3, a_4$. Each cell contains the corresponding product\ldots}
\end{figure}

Brahmagupta's \textit{Br\={a}hmasphu{\d{t}}asiddh\={a}nta} (628 AD) contained the two-square identity centuries before European mathematicians rediscovered it. Fibonacci popularized it in his \textit{Liber Quadratorum} (1225) --- "The Book of Squares" --- a masterpiece of medieval number theory that was almost entirely ignored by his contemporaries, who preferred his \textit{Liber Abaci} and the famous rabbit sequence. Euler proved the four-square version in 1748, and it was essential to Lagrange's 1770 proof that every positive integer is a sum of four squares.
...
\caption{A Venn diagram with three large overlapping circles. The left circle is labeled "Pythagorean $k$-tuples", the right circle "Modern factoring algorithms (QS, NFS)", and the bottom circle "Congruences\ldots}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../Chapter6/images/fig15_rho_orbit.png}
\caption{A number line from $0$ to $N - 1$ showing the squaring map orbit: dots at positions $x, x^2 \bmod N, x^4 \bmod N, x^8 \bmod N, \ldots$, with arrows connecting successive values. Two dots at different\ldots}
\end{figure}
...
\caption{An octopus in a garden of numbers, each tentacle reaching into a different "keyhole" in a giant composite number $N$ inscribed on a stone vault embedded in the ground. The octopus is smiling\ldots}
\end{figure}
