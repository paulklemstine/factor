...
\caption{A whimsical "magic box" diagram. Three labeled machines --- $A$, $B$, $C$ --- each drawn as brass-and-gear contraptions in a Victorian style, reminiscent of Babbage's Difference Engine. A triple\ldots}
\end{figure}
...
\caption{A small table showing 5 concrete numerical examples. Columns: "Original $(a,b,c)$", "$Q$", "Transformed $(a', b', c')$", "$Q'$". Each row shows a different triple --- including one negative and one\ldots}
\end{figure}
...
\caption{A 3D rendering of the cone $a^2 + b^2 = c^2$ in $(a,b,c)$-space, viewed slightly from above and to the right. The cone opens upward along the positive $c$-axis. Integer lattice points on the cone's\ldots}
\end{figure}
...
\begin{quote}
\itshape \textit{The Berggren transformations map Pythagorean triples to Pythagorean triples.}
\end{quote}
...
\begin{itemize}
\item **Recipe $A$:** $(a - 2b + 2c)^2 + (2a - b + 2c)^2 = (2a - 2b + 3c)^2$
\item **Recipe $B$:** $(a + 2b + 2c)^2 + (2a + b + 2c)^2 = (2a + 2b + 3c)^2$
\item **Recipe $C$:** $(-a + 2b + 2c)^2 + (-2a + b + 2c)^2 = (-2a + 2b + 3c)^2$
\end{itemize}
...
\begin{quote}
\itshape \textit{Every primitive Pythagorean triple appears exactly once in the Berggren tree.}
\end{quote}
...
\caption{The first four levels of the Berggren tree, drawn as an elegant ternary tree diagram on cream paper. Root node: a small right triangle drawn to scale inside a circle, labeled $(3,4,5)$. First\ldots}
\end{figure}
...
\caption{A side-by-side comparison. LEFT: A Minkowski spacetime diagram showing a light cone opening upward and downward from the origin, with two events marked on the upper cone connected by a curved arrow\ldots}
\end{figure}
...
Let me approach this differently and with more discipline. The triple $(697, 696, 985)$ satisfies $697^2 + 696^2 = 485809 + 484416 = 970225 = 985^2$. Indeed, $985^2 = 970225$. \ensuremath{\checkmark}

Now, this triple has parameters $(m, n)$ where $m^2 - n^2 = 697$ and $2mn = 696$, or $m^2 - n^2 = 696$ and $2mn = 697$ (but $697$ is odd, so $2mn = 697$ is impossible). So $m^2 - n^2 = 697$ and $2mn = 696$, giving $mn = 348$. From $m - n = 697/(m+n)$ and $mn = 348$, we can solve: trying $m = 29, n = 12$: $29^2 - 12^2 = 841 - 144 = 697$ \ensuremath{\checkmark}, and $2 \cdot 29 \cdot 12 = 696$ \ensuremath{\checkmark}. So $(m, n) = (29, 12)$.
...
\caption{A vertical "elevator" diagram. The triple $(697, 696, 985)$ is at the top floor. At each floor below, an inverse Berggren matrix is applied (labeled with which inverse, $A^{-1}$, $B^{-1}$, or\ldots}
\end{figure}
...
\caption{A portion of the Berggren tree showing only the "A-highway" --- the leftmost branch at every generation, drawn as a long straight road receding into one-point perspective. Each node is a signpost\ldots}
\end{figure}
...
\begin{itemize}
\item $p = 5$: depth $= \frac{5-3}{2} = 1$. Triple $(5, 12, 13)$, one $A$-step from root. \ensuremath{\checkmark}
\item $p = 7$: depth $= \frac{7-3}{2} = 2$. Triple $(7, 24, 25)$, two $A$-steps from root. \ensuremath{\checkmark}
\item $p = 11$: depth $= \frac{11-3}{2} = 4$. Triple $(11, 60, 61)$, four $A$-steps from root. \ensuremath{\checkmark}
\item $p = 13$: depth $= \frac{13-3}{2} = 5$. Triple $(13, 84, 85)$, five $A$-steps from root. \ensuremath{\checkmark}
...
\caption{A vertical number line on the left showing primes $5, 7, 11, 13, 17, 19, 23, 29, 31, \ldots$ in ascending order. Horizontal arrows of increasing length connect each prime to its depth on a scale at\ldots}
\end{figure}
...
\midrule
$1$ & $225$ & $113$ & $112$ & $(15, 112, 113)$ & \ensuremath{\checkmark} \\
$3$ & $75$ & $39$ & $36$ & $(15, 36, 39)$ & \ensuremath{\times} (gcd = 3) \\
$5$ & $45$ & $25$ & $20$ & $(15, 20, 25)$ & \ensuremath{\times} (gcd = 5) \\
$9$ & $25$ & $17$ & $8$ & $(15, 8, 17)$ & \ensuremath{\checkmark} \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{A "multiplication table" style grid. Rows labeled with the four small-factor divisors of $225$: $1, 3, 5, 9$. Columns labeled with their complementary large factors: $225, 75, 45, 25$. In each cell,\ldots}
\end{figure}
...
\begin{tabular}{c c}
\toprule
$k$ (number of prime factors) & $\vert T(N)\vert $ \\
\midrule
$1$ (prime) & $1$ \\
$2$ (semiprime) & $4$ \\
$3$ ($3$-almost-prime) & $13$ \\
...
\caption{A bar chart showing $|T(N)|$ for $k = 1, 2, 3, 4, 5$ distinct odd prime factors. The bars grow exponentially: $1, 4, 13, 40, 121$. Each bar is decorated with tiny right triangles to convey the count\ldots}
\end{figure}
...
\caption{A Poincar\'e disk tessellation in the spirit of Escher's \textit{Circle Limit III}. Instead of fish, each tile contains a right triangle with its side lengths printed inside. At the center: a large\ldots}
\end{figure}
...
\caption{A full-page "Grand Diagram." At the center, a right triangle labeled $(a, b, c)$ with a small square at the right angle and $Q = 0$ inscribed inside it. Radiating outward in four directions from the\ldots}
\end{figure}
...
\item **The $A$-highway.** The prime $p = 29$ has depth $\frac{29 - 3}{2} = 13$. Write down the corresponding Pythagorean triple and verify that it lies on the $A$-highway.
...
\item \textbf{\ensuremath{\star} (Challenge)} Show that the Berggren matrix $B$ satisfies $B^\top J B = J$, where $J = \text{diag}(1, 1, -1)$. *(This proves that $B$ is an element of the Lorentz group $O(2,1)$.)*
\item \textbf{\ensuremath{\star} (Challenge)} Prove the identity $(a + 2b + 2c)^2 + (2a + b + 2c)^2 - (2a + 2b + 3c)^2 = a^2 + b^2 - c^2$ by direct expansion. Conclude that $B$ preserves $Q$ for \textit{all} triples, not just Pythagorean ones.
\item \textbf{\ensuremath{\star}\ensuremath{\star} (Research)} Is there a four-dimensional analogue? That is, can one construct a tree of primitive Pythagorean \textit{quadruples} $(a, b, c, d)$ with $a^2 + b^2 + c^2 = d^2$, generated by integer matrices in $O(3,1; \mathbb{Z})$? If so, how many generators are needed?
\item \textbf{\ensuremath{\star}\ensuremath{\star} (Open-ended)} The depth formula $\text{depth}(p) = (p-3)/2$ tells us that large primes live deep in the tree. Is there a useful sense in which the \textit{difficulty} of factoring a number $N$ is related to the depth of some associated Pythagorean triple? Explore this question computationally for small semiprimes.
\item \textbf{A meditation.} The quadratic form $Q(a, b, c) = a^2 + b^2 - c^2$ appears in at least three independent contexts: Pythagorean triples, special relativity, and hyperbolic geometry. Can you think of a fourth?
\end{enumerate}
