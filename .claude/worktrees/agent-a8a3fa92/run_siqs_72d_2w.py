...
N, p, q = make_semiprime(72)
print(f"{'='*60}", flush=True)
print(f"SIQS 72d (2 workers): N = {str(N)[:40]}...", flush=True)
...
if result and result != N and N % result == 0:
    print(f"\nSUCCESS: 72d (2w) in {elapsed:.1f}s", flush=True)
else:
    print(f"\nFAILED: 72d (2w) after {elapsed:.1f}s", flush=True)
