...
void ec_kang_geom_init(const char *p_hex, const char *order_hex) {
    if (!inited) {
...
/* --- Geometric jump table: jumps[i] = round(r^i), r = (2*mean)^(1/63) ---
 * Designed so jumps[0]=1, jumps[63]=2*mean_target, geometrically spaced.
 * These are base values before scaling (same as PYTH_HYPS role).
 * Base mean ~10765 to match original PYTH_HYPS mean (~10765).
 */
 ...
 */
int ec_kang_geom_solve_ex(const char *Gx_hex, const char *Gy_hex,
...
int ec_kang_geom_solve(const char *Gx_hex, const char *Gy_hex,
                  const char *Px_hex, const char *Py_hex,
                  const char *search_bound_hex,
                  char *result, size_t result_size) {
    return ec_kang_geom_solve_ex(Gx_hex, Gy_hex, Px_hex, Py_hex,
