...
/* Z/6 equivalence class constants for secp256k1.
 * β is a cube root of unity mod p: β³ ≡ 1 (mod p), β ≠ 1.
 * φ(x,y) = (β·x, y) is an automorphism with φ(P) = [λ]P.
 * Combined with negation [-1], we get 6-fold equivalence classes.
 * Storing canonical x-coord (min of x, β·x, β²·x) triples collision rate.
 * With negation: 6x collision rate → √6 ≈ 2.45x speedup. */
static const h_fe_t Z6_BETA  = {{ 0xc1396c28719501ee, 0x9cf0497512f58995,
                                   0xac3434e99cf04975, 0x7ae96a2b657c0710 }};  /* not right order */
static const h_fe_t Z6_BETA2 = {{ 0, 0, 0, 0 }};  /* computed at init */

/* λ and λ² mod n (group order) for scalar adjustment on collision */
static const unsigned __int128 Z6_LAMBDA  = ((unsigned __int128)0x5363ad4cc05c30e0ULL << 64)
                                           | 0xa5261c028812645aULL;
static const unsigned __int128 Z6_LAMBDA2 = ((unsigned __int128)0ULL << 64) | 0ULL; /* computed */

/* secp256k1 group order n */
static const unsigned __int128 Z6_N_HI = 0xFFFFFFFFFFFFFFFEULL;
static const unsigned __int128 Z6_N_LO = 0xBAAEDCE6AF48A03BULL;

static h_fe_t g_beta, g_beta2;
static int g_z6_initialized = 0;

static void z6_init(void) {
    if (g_z6_initialized) return;
    /* β for secp256k1 (cube root of unity mod p) */
    g_beta.v[0] = 0xc1396c28719501eeULL;
    g_beta.v[1] = 0x9cf0497512f58995ULL;
    g_beta.v[2] = 0xac3434e99cf04975ULL;
    g_beta.v[3] = 0x7ae96a2b657c0710ULL;
    /* Wait — need to verify byte order. The hex is:
     * 0x7ae96a2b657c07106e64479eac3434e99cf0497512f58995c1396c28719501ee
     * In 4 x uint64 little-endian limbs:
     * v[0] = lowest 64 bits = c1396c28719501ee
     * v[1] = next            = 9cf0497512f58995  -- WRONG, let me reparse */
    /* Full hex: 7ae96a2b657c0710 6e64479eac3434e9 9cf0497512f58995 c1396c28719501ee */
    g_beta.v[3] = 0x7ae96a2b657c0710ULL;
    g_beta.v[2] = 0x6e64479eac3434e9ULL;
    g_beta.v[1] = 0x9cf0497512f58995ULL;
    g_beta.v[0] = 0xc1396c28719501eeULL;

    /* β² = β * β mod p */
    h_fe_mul(&g_beta2, &g_beta, &g_beta);
    g_z6_initialized = 1;
}

/* Compare two 256-bit field elements. Returns -1, 0, or 1. */
static int h_fe_cmp(const h_fe_t *a, const h_fe_t *b) {
    for (int i = 3; i >= 0; i--) {
...
/* Compute canonical x-coordinate: min(x, β·x mod p, β²·x mod p).
 * This maps all 3 points in a Z/3Z orbit to the same key. */
static void z6_canonical_x(h_fe_t *canon, const uint64_t x[4]) {
    h_fe_t xfe, bx, b2x;
    memcpy(xfe.v, x, 32);

    h_fe_mul(&bx, &xfe, &g_beta);
    h_fe_mul(&b2x, &xfe, &g_beta2);

    /* Pick the smallest */
    h_fe_set(canon, &xfe);
    if (h_fe_cmp(&bx, canon) < 0) h_fe_set(canon, &bx);
    if (h_fe_cmp(&b2x, canon) < 0) h_fe_set(canon, &b2x);
}

typedef struct cpu_dp_entry {
    uint64_t x[4];       /* canonical x-coordinate */
    uint64_t orig_x[4];  /* original x for verification */
    uint64_t pos_lo;
    uint64_t pos_hi;
    int is_tame;
...
static void cpu_dp_insert(cpu_dp_table_t *t, const uint64_t x[4], uint64_t pos_lo, uint64_t pos_hi, int is_tame) {
    /* Z/6: store canonical x-coordinate for equivalence class matching */
    h_fe_t canon;
    z6_canonical_x(&canon, x);
    uint64_t h = canon.v[0] % CPU_DP_TABLE_SIZE;
    cpu_dp_entry_t *e = (cpu_dp_entry_t *)malloc(sizeof(cpu_dp_entry_t));
    e->x[0] = canon.v[0]; e->x[1] = canon.v[1]; e->x[2] = canon.v[2]; e->x[3] = canon.v[3];
    e->orig_x[0] = x[0]; e->orig_x[1] = x[1]; e->orig_x[2] = x[2]; e->orig_x[3] = x[3];
...
static cpu_dp_entry_t *cpu_dp_find(cpu_dp_table_t *t, const uint64_t x[4], int is_tame) {
    /* Z/6: look up by canonical x-coordinate */
    h_fe_t canon;
    z6_canonical_x(&canon, x);
    uint64_t h = canon.v[0] % CPU_DP_TABLE_SIZE;
    ...
    while (e) {
        if (e->is_tame != is_tame &&
            e->x[0] == canon.v[0] && e->x[1] == canon.v[1] &&
            e->x[2] == canon.v[2] && e->x[3] == canon.v[3])
            return e;
        e = e->next;
    }
    return NULL;
...
int ec_kang_gpu_solve(const char *Gx_hex, const char *Gy_hex,
    ...
    h_fe_t Gx, Gy, Px, Py;
    z6_init();  /* Initialize Z/6 equivalence class constants */
